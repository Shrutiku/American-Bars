     		...
     		<div class="margin-top-50 bg_brown">
     			...
     			<div class="dashboard_detail">
     				<div class="result_search event"><div class="result_search_text pull-left"><i class="strip events"></i> Message Subject : <?php echo @ucfirst($subject);?></div><div class="pull-right">
     					<a href="<?php echo site_url('bar/list_message')?>" class="white pull-right review">Back</a></div><div class="clearfix"></div></div>
		     		...
		     		<div class="dashboard_subblock">
		     			...
		     			<?php if($from_user_type=='admin'){
		     				$user_info = get_admin_single_detail($from_user_id);
																	
																	$name1 = $user_info -> first_name." ".$user_info->last_name;
																	$profile_image1 = $user_info -> image;
																	if($profile_image1 != "" && file_exists(base_path()."/upload/admin/".$profile_image1))
																	{
																		$profile_image1 = '/upload/admin/'.$profile_image1;
																	}
																	else
																	{
     						...
     						<?php } else {
     							...
     							$user_info = get_admin_single_detail($val->from_user_id);
																	...
																	if($profile_image != "" && file_exists(base_path()."/upload/admin/".$profile_image))
																	{
																		$profile_image = '/upload/admin/'.$profile_image;
																	}
																	else
																	{
																		$profile_image = '/upload/no-image.png';
...
window.setInterval(function(){
   ...
   var list = $('.chats', cont);
	$.ajax({
		url:'<?php echo site_url('bar/getTicketCommentAjax'); ?>',
		type:'POST',
  ...
  /// call your function here
	...
	$('#post').click(function(){
		 ...
		 var text = $('#comment').val(); 
             ...
             if(text!=''){   
				$.ajax({
					url:'<?php echo site_url('bar/postConversationReply'); ?>',
					type:'post',
					dataType:'Json',
