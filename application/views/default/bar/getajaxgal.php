			...
			<!-- <form class="form-horizontal" id="bar-search-frm" method="post" role="form" action="http://192.168.1.27/ADB/bar/lists"> -->
               <!-- <div class="form-group pull-right"> -->
               	<ul class="social_icon pull-right">
						   ...
						   <li><a href="javascript://" onclick="fbShare('<?php echo $i; ?>')"><img src="<?php echo base_url().'default'?>/images/result_fb.png"></a></li>
						    <li><a onclick="twShare('<?php echo $i; ?>')" href="javascript://"><img src="<?php echo base_url().'default'?>/images/result_twitt.png"></a></li>
						    <li><a onclick="gPlusShare1('<?php echo site_url('bar/gallery/'.$i)?>','john deo')" href="javascript://"><img src="<?php echo base_url().'default'?>/images/result_google.png"></a></li>
						    <!-- <li><a onclick="inShare()" href="javascript://"><img src="<?php echo base_url().'default'?>/images/result_linkln.png"></a></li> -->
						     <li><a  href="javascript://" onclick="piShare('<?php echo $i; ?>')"><img src="<?php echo base_url().'default'?>/images/result_p.png" /></a></li>
...
<script>
	function fbShare(id){
	window.open('https://www.facebook.com/sharer/sharer.php?u='+encodeURIComponent('<?php echo site_url('bar/gallery/'); ?>/'+id),'facebook-share-dialog','width=626,height=436');
            return false;
}
...
function twShare(id)
{
	var width  = 575,
        ...
        url    = 'http://twitter.com/share?url=<?php echo site_url('bar/gallery'); ?>/'+id,
        opts   = 'status=1' +
                 ',width='  + width  +
                 ',height=' + height +
                 ',top='    + top    +
...
function piShare(id)
{
	var width  = 600,
        height = 500,
         left   = ($(window).width()  - width)  / 2,
         top    = ($(window).height() - height) / 2,
        url    = 'http://www.pinterest.com/pin/create/button/?url=<?php echo site_url('bar/gallery'); ?>/'+id+'&media=<?php echo $img; ?>&description=<?php //echo $bar_detail['bar_desc']; ?>',
        opts   = 'status=1' +
                 ',width='  + width  +
                 ',height=' + height +
                 ',top='    + top    +
