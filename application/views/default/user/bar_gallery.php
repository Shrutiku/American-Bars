...
$('.sorted_table').sortable({
  ...
  onDrop: function (item, container, _super) {
         ...
         console.log(ids);
           ...
           $.ajax({
                url: "<?php echo site_url('user/reorder');?>",
                type: "post",
                data: {id:ids},
                cache: false,
                dataType: "json",
  		...
  		<div class="container">
     		<div class="bg_brown">
				     				...
				     				<form class="form-horizontal" role="form">
					                   <div class="form-group">
					                       <div class="col-sm-8 input_box pull-left" style="padding: 0 5px 0 0;">
					                           <input type="text" name="event_keyword" id="event_keyword" class="form-control form-pad"  placeholder="Search My Albums" onkeydown="if (event.keyCode == 13) { get_search_event(); return false; }" />
					                       </div>
					                       <div class="col-sm-2 input_box pull-left" style="margin-right: 0;">
				                       	   ...
				                       	   <div class="col-sm-2 input_box pull-left" style="margin-right: 0;">
				                        		<a href="<?php echo site_url('user/album')?>" class="btn btn-lg btn-primary search" type="submit"><i class="strip refresh"></i>
				                       	   </a></div>
					...
					<input type="hidden" name="action" id="action" />
	     				...
	     				<?php echo $page_link;?>
							...
							<table  class="table  sorted_table table border">
								...
								<?php $i++; } } else { ?>
									<tr>
										<td colspan="5">You do not have an album.</td>
									</tr>	
				...
				<div id="list_show" style="display: none;" >	
					...
					<form name="add_event" id="form" method="post" enctype="multipart/form-data" action="<?php echo site_url('user/add_album/'.base64_encode(@$getbar['bar_id'])); ?>">
						<input type="hidden" name="bar_gallery_id" id="bar_gallery_id" value="" />
	                     ...
	                     <div id="hide_edit">
	                     	<div id="inner">  	
	                       	<div class="padtb">
	        				 	<div class="col-sm-3 text-right">
	        				 		<label class="control-label">Gallery Image 1 : <span class="aestrick"> * </span></label>
	        				 	</div>
	                       		...
	                       		<div class="padtb">
	        				 	<div class="col-sm-3 text-right">
	        				 		<label class="control-label">Image Title 1: </label>
	        				 	</div>
	                       		<div class="input_box col-sm-7">
	                       	...
	                       	<div class="padtb8">
	                       		...
	                       		<div class="col-sm-7 mart10 text-left">
	                       			...
	                       			<a  class="btn btn-lg btn-primary marr_10" href="<?php echo site_url('user/album');?>" >Cancel</a>
	                       		</div>
	                       		<div class="clearfix"></div>
	                       	</div>
	                       	<div class="clearfix"></div>
...
<script type="text/javascript" src="<?php echo base_url().getThemeName(); ?>/js/jquery.easing.1.3.js"></script>
   ...
   $("form#form").submit(function(){
    ...
    $.ajax({
        url: '<?php echo site_url('user/add_album/');?>',
        type: 'POST',
        data: formData,
        async: false,
         dataType : 'json',
			   ...
			   complete: function(){
		    	...
		    	success : function ( json ) 
		    	{
					...
					else
					{
						...
						if($('#event_id').val()=='')
						{
							$.growlUI('Your album add successfully .');
						}
						else
						{
    ...
    function list_add()
    {
    	...
    	$(':input','#form')
					 	...
					 	.removeAttr('selected');
					 	 $.ajax({
			       type: "POST",
				   url: "<?php echo site_url('user/bargalleryimages')?>",
...
$(document).ready(function(){	
 ...
 function editbargallery(id)
 {
 	 ...
 	 $.ajax({
	       type: "POST",
		   url: "<?php echo site_url('user/bargallerydetail')?>",
		   data : {id:id},
		   dataType : 'JSON',
		   success: function(response) {
 ...
 function get_search_event()
 {
 	  ...
 	  var res = $.ajax(
	        {						
			   type: 'POST',
			   url: '<?php echo site_url('user/album/')?>',
			   dataType: 'post', 
 ...
 function deletegallery(id)
 {
	        ...
	        {						
			   type: 'POST',
			   url: '<?php echo site_url('user/bargallerydelete/')?>',
			   dataType: 'post', 
			   data : {id:id},
			   cache: false,
			   async: false,
...
$('#add_row').click(function(){
		...
		$('#cntpro').val(cnt)
		$('#inner').append('<div id="img_'+cnt+'" style="display:none;"><div class="padtb" ><div class="col-sm-3 text-right"><label class="control-label"></label></div><div class="input_box upload_btn" data-provides="fileupload"><div class="uneditable-input"><i class="icon-file fileupload-exists"></i><span class="fileupload-preview"></span></div><div class="input_box upload_btn"><span class="fileupload-exists"></div><input accept="image/*" type="file" class="form-control form-pad" name="photo_image[]" id="photo_image_'+cnt+'" /></span></div><a href="javascript://" class="btn btn-lg btn-primary search marr_10 pull-left" onclick="removeImageDive(\''+cnt+'\')"><span class="glyphicon glyphicon-minus"></span></a></div><div class="clear"></div><div class="padtb"><div class="col-sm-3 text-right"><label class="control-label">Image Title '+cnt+' : </label></div><div class="input_box col-sm-7"><input type="text" class="form-control form-pad" id="image_title" name="image_title[]" value=""></div><div class="clearfix"></div></div></div>');
		$('#img_'+cnt).slideDown();
			
		});
		
		
function removeImageDive(id,id1)
	{
	...
	//	$('#img_'+id).remove();
		...
		for (i =id; i <= $('#cntpro').val(); i++){
			     $(".salman"+i).html(parseInt(i)-1);
			     //$(".salman").html(i);
			      //alert(i);
		}
		$('#cntpro').val(cnt);
	...
	function removeImageDiveAjax(id)
	{
		...
		alertify.confirm("Are you sure you want to delete this gallery image?", function (e) {
			if (e) {
			$.ajax({
				url:'<?php echo site_url('user/removeImageAjax') ?>/'+id,
