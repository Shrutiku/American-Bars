<div class="wrapper row6 padtb10 has-js">
  		...
  		<div class="container">
     		<div class="bg_brown">
     			...
     			<div class="dashboard_detail">
     				<div class="result_search event"><div class="result_search_text"><i class="strip liquor"></i> Favorite Liquor</div></div>
		     		<div class="dashboard_subblock">
		     			<div>
		     				<div class="dash-btngroup">
	     						<div id="hd_del">
	     							<a class="btn btn-lg btn-primary marr_10" href="javascript:void(0)" onclick="searchboxliquor();">Add Favorite Liquors</a>
		                       		<a class="btn btn-lg btn-primary marr_10" href="javascript:void(0)" onclick="setaction('chk[]','delete',   'actionliquorfav');">Delete</a>
	     						</div>
     						</div>
     					<div id="list_hide_m">
     						<?php			 
				     				...
				     				<form class="form-horizontal" role="form">
					                   <div class="form-group">
					                       <div class="col-sm-8 input_box pull-left" style="padding: 0 5px 0 0;">
					                           <input type="text" name="event_keyword" id="event_keyword" class="form-control form-pad"  placeholder="Search My Favorite Liqour" onkeydown="if (event.keyCode == 13) { get_search_event(); return false; }" />
					                       </div>
					                       <div class="col-sm-2 input_box pull-left" style="margin-right: 0;">
				                       	   ...
				                       	   <div class="col-sm-2 input_box pull-left" style="margin-right: 0;">
				                        		<a href="<?php echo site_url('user/favoriteliquor')?>" class="btn btn-lg btn-primary search" type="submit"><i class="strip refresh"></i>
				                       	   </a></div>
							...
							<table class="table">
								...
								if ($i % 2 == 0)
									...
									<tr class="<?php echo $dark; ?>" id='remove_event_<?php echo $event->liquor_id; ?>'>
										<td><label class="radio-checkbox label_check c_on" for="checkbox-<?php echo  $event->liquor_id;?>">
											<input type="checkbox"  class="checkboxes" name="chk[]" id="checkbox-<?php echo  $event->liquor_id;?>" value="<?php echo $event->liquor_id;?>"></label></td>
										
										<td><a  class="bar_title" href="<?php echo site_url('liquor/detail/'.$event->liquor_slug);?>"><?php echo $event->liquor_title;?></td>
										<td><?php echo $event->type;?></td>
										<td><?php 
								       	  
								       	  echo  date($site_setting->date_format. " h:i:s",strtotime($event->date)); ?></td>
										<!-- <td><?php echo $event->country;?></td> -->
										<td>
											<!-- <a href="javascript://" onclick="editbarevent('<?php echo $event->event_id; ?>')"><i class="strip edit_table"></i></a> -->
											<a href="javascript://" onclick="deletefavliquor('<?php echo $event->liquor_id; ?>')" ><i class="strip delete"></i></a>
										</td>
									</tr>
								<?php $i++; } } else { ?>
									<tr>
										<td colspan="7">You have no favorite liqours.</td>
									</tr>	
 ...
 <div class="modal fade login_pop2" id="searchboxbeer" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" >
	  <?php echo $this->load->view(getThemeName().'/home/searchboxliquor');?>	
</div>
<script>
	function searchboxliquor()
	{
		$("#searchboxbeer").modal('show');
		 
	}
function deletefavliquor(id)
 {
 	 
			alertify.confirm("Are you sure you want to delete this favorite liquor?", function (e) {
				if (e) {
					 var res = $.ajax(
	        {						
			   type: 'POST',
			   url: '<?php echo site_url('liquor/deletefavliquor/')?>',
			   dataType: 'post', 
			   data : {id:id},
			   cache: false,
			   async: false,
			   beforeSend : function(){
				...
				complete: function(){
				     ...
				     $.growlUI('Your favorite liquor deleted successfully .');
				    }
				}).responseText;
				bindJquery();
			}
...
function setupLabel() {
 ...
 function get_search_event()
 {
 	  ...
 	  var res = $.ajax(
	        {						
			   type: 'POST',
			   url: '<?php echo site_url('user/favoriteliquor/')?>',
			   dataType: 'post', 
