...
<script type="text/javascript" language="javascript">
	...
	function delete_rec(id,redirectpage,option,keyword,limit,offset)
	{
		...
		if(ans)
		{
			location.href = "<?php echo base_url(); ?>video/delete_video/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+limit+"/"+offset;
		}else{
			return false;
		}
	}
	...
	function getlimit(limit)
	{
		...
		{
			window.location.href='<?php echo base_url();?>video/list_video/'+limit+'/';
		}
	...
	function getsearchlimit(limit)
	{
		...
		else
		{
			...
			window.location.href='<?php echo base_url();?>video/search_list_video/'+limit+'/<?php echo $option.'/'.$keyword; ?>';
		}
	...
	function gomain(x)
	{
		...
		if(x == 'all')
		{
			window.location.href= '<?php echo base_url();?>video/list_video';
		}
...
<div class="page_content">
			...
			<div class="container_fluid">
				...
				<div class="row_fluid"> 
					<h3 class="page_title">Video Listing</h3>
    ...
    <?php } ?>
				<div class="row_fluid"> 
					...
					echo form_open('video/search_list_video/'.$limit,$attributes);?>
					<div class="fl_left">
					...
					<?php echo anchor('video/add_video','Add New', 'class="btn blue  fl_left mar_r_5" id="addvideo"'); ?>
					...
					<div class="clear"></div>		
									...
									<div class="clear"></div>
			<form name="frm_listlogin" id="frm_listlogin" action="<?php echo base_url();?>video/action_video" method="post">
	<input type="hidden" name="offset" id="offset" value="<?php echo $offset; ?>" />
				   ...
				   <div class="scroll-pane horizontal-only" >
									<table class="table border" >
												...
												<th class="sorting_disabled" style="width: 6%;">Video Title</th>
												<th class="sorting" style="width: 12%;">Video Category</th>
												<th class="sorting" style="width: 12%;">User Name</th>
												<th class="sorting" style="width: 18%;">Video Price</th>
												<th class="sorting" style="width: 5%;">Video Type</th>
								...
								if($result)
								{
									...
									<td  style="width: 30px;"><input type="checkbox" name="chk[]" value="<?php echo $row->video_id ?>" class="chk"  /></td>
														<td class=" sorting_1"><?php echo $row->video_title; ?></td>
														...
														<td><?php echo get_video_rating($row->video_id); ?></td>
														<td class="center">
															<?php $cls = ($row->video_status=='active')?'purple':'yellow';?>
															<span class="<?php echo $cls;?>"><?php echo ucfirst($row->video_status); ?></span>
														</td>
														
														<!-- <td class="center"><?php if($this->session->userdata('video_type')=='1'){echo anchor('rights/assign_rights/'.$row->video_id.'/'.$limit.'/'.$offset,'<i class="halflings-icon ok"></i>'); }else{ echo "N/A";} ?></td>
														<td>
														<a class="table_icon btn green" href="<?php echo site_url('video/list_logactivity/'.$row->video_id);?>">
														...
														<?php 
																...
																echo anchor('video/edit_video/'.$row->video_id.'/'.$redirect_page.'/'.$option.'/'.$keyword.'/'.$limit.'/'.$offset,'<i class="comon_icon edit_icon"></i>','class="table_icon btn blue" id="video_'.$row->video_id.'" title="Edit Admin"'); 
														?>
														
														<a class="table_icon btn red" href="javascript://" onClick="delete_rec('<?php echo $row->video_id; ?>','<?php echo $redirect_page;?>','<?php echo $option?>','<?php echo $keyword?>','<?php echo $limit?>','<?php echo $offset; ?>')" title="Delete"><i class="comon_icon delete_icon"></i></a>
														</td>
													</tr>
								<?php $i++;} }else{ ?>
