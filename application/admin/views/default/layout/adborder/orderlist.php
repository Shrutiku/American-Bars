...
function gomain(x)
	{
		...
		if(x == 'all')
		{
			window.location.href= '<?php echo site_url('adborder/orderlist');?>';
		}
...
function getData(limit,offset)
{
	...
	var url='<?php echo site_url('adborder') ?>/'+redirect_page+'/'+limit+'/'+offset;
	}else{
		var url='<?php echo site_url('adborder/') ?>/'+redirect_page+'/'+limit+'/<?php echo $option.'/'.$keyword.'/'; ?>'+offset;
	 } 
	// alert(url);
...
function delete_rec(id,redirectpage,option,keyword,from_date,to_date,limit,offset)
	{
		...
		if(ans)
		{
			location.href = "<?php echo base_url(); ?>adborder/deleteOrder/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+from_date+"/"+to_date+"/"+limit+"/"+offset;
...
function getlimit(limit)
	{
		...
		else
		{
			...
			$.ajax({
			url : '<?php echo site_url('adborder/orderlist/') ?>/'+limit,
			beforeSend : function() {
			},
	...
	function getsearchlimit(limit)
	{
		...
		{
			window.location.href='<?php echo base_url();?>adborder/searchOrderlist/'+limit+'/<?php echo $option.'/'.$keyword.'/'.@$from_date.'/'.@$to_date; ?>';
		}
	...
	function statusChange(order_id,order_status){
		$.ajax({
			url : '<?php echo site_url('adborder/statusChange');?>/'+order_id+'/'+order_status+'/',
			beforeSend : function() {
			},
			success : function(data) {
				if(data=='done'){
...
<div class="page_content">
			...
			<div class="container_fluid">
				...
				<div class="row_fluid"> 
					<h3 class="page_title">Order Listing</h3>
				...
				<div class="row_fluid"> 
					<div class="portlet blue">
                        ...
                        <?php } ?>
                                ...
                                <option value="1" <?php if($limit==1){?> selected="selected"<?php }?>>5</option>
					...
					echo form_open('adborder/searchOrderlist/'.$limit,$attributes);?>
					<div class="fl_left">
					...
					<div class="clear"></div>		
									...
									<div class="clear"></div>
			<form name="frm_listlogin" id="frm_listlogin" action="<?php echo base_url();?>adborder/actionOrder" method="post">
	<input type="hidden" name="offset" id="offset" value="<?php echo $offset; ?>" />
				   ...
				   <div class="scroll-pane horizontal-only" >
						...
						<?php } ?>	
														...
														<td><a target="_blank" href="<?php echo base_url().'adborder/print_order/'.$row->order_id ?>">Print</a></td>
														
														<td>
														
														 <a title="View" class="table_icon btn blue" href="<?php echo site_url('adborder/orderDetails/'.$row->order_id.'/'.$limit.'/'.$option.'/'.$keyword.'/'.$offset); ?>" ><i class="comon_icon view_icon"></i></a>
