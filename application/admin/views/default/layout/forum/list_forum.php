...
<script type="text/javascript" language="javascript">
	...
	function delete_rec(id,redirectpage,option,keyword,limit,offset)
	{
		...
		if(ans)
		{
			location.href = "<?php echo base_url(); ?>forum/delete_forum/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+limit+"/"+offset;
		}else{
			return false;
		}
	}
	...
	function getlimit(limit)
	{
		...
		{
			window.location.href='<?php echo base_url();?>forum/list_forum/'+limit+'/';
		}
	...
	function getsearchlimit(limit)
	{
		...
		else
		{
			...
			window.location.href='<?php echo base_url();?>forum/search_list_forum/'+limit+'/<?php echo $option.'/'.$keyword; ?>';
		}
	...
	function gomain(x)
	{
		...
		if(x == 'all')
		{
			window.location.href= '<?php echo base_url();?>forum/list_forum';
		}
...
function setaction(elename, actionval, actionmsg, formname) {
	...
	function reply_message(id,redirectpage,option,keyword,limit,offset){
		location.href = "<?php echo base_url(); ?>forum/view/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+limit+"/"+offset;
	}
...
<div class="page_content">
			...
			<div class="container_fluid">
				...
				<div class="row_fluid"> 
					<h3 class="page_title">Forum Listing</h3>
    ...
    <?php } ?>
				<div class="row_fluid"> 
					...
					echo form_open('forum/search_list_forum/'.$limit,$attributes);?>
					<div class="fl_left">
					...
					<span class="sspan fl_left ">&nbsp;&nbsp;&nbsp;Search By</span>&nbsp;
                            <select name="option" id="option" onchange="gomain(this.value)"  class="m_wrap fl_left mar0" style="padding:6px;">
                                ...
                                <option value="topic_name" <?php if($option=='topic_name'){?> selected="selected"<?php }?>>Topic name</option>
							...
							<input type="button" name="refresh" id="submit" value="Refresh" class="btn blue  fl_left mar10" onclick="document.location.href ='<?php echo site_url("forum/list_forum/"); ?>'" />
								</form>
								</div>
									<div class="fl_right">
					<?php echo anchor('forum/add_forum','Add New', 'class="btn blue  fl_left mar_r_5" id="addforum"'); ?>
					...
					<div class="clear"></div>		
									...
									<div class="clear"></div>
			<form name="frm_listlogin" id="frm_listlogin" action="<?php echo base_url();?>forum/action_forum" method="post">
				   ...
				   <div class="scroll-pane horizontal-only" >
								...
								if($result)
								{
									...
									<td  style="width: 30px;"><input type="checkbox" name="chk[]" value="<?php echo $row->forum_id ?>" class="chk"  /></td>
														<td class=" sorting_1"><?php echo $row->topic_name; ?></td>
														<td class=" sorting_1"><?php echo $row->forum_category_name; ?></td>
														<td align="center" style="text-align:center;">
														<a class="table_icon btn blue" href="javascript://" onclick="reply_message('<?php echo $row->forum_id; ?>','<?php echo $redirect_page;?>','<?php echo $option?>','<?php echo $keyword?>','<?php echo $limit?>','<?php echo $offset; ?>');" title="View"><i class="comon_icon view_icon"></i></a>
														</td>
														<td class="center">
														...
														<?php 
																...
																echo anchor('forum/edit_forum/'.$row->forum_id.'/'.$redirect_page.'/'.$option.'/'.$keyword.'/'.$limit.'/'.$offset,'<i class="comon_icon edit_icon"></i>','class="table_icon btn blue" id="forum_'.$row->forum_id.'" title="Edit Admin"'); 
														?>
