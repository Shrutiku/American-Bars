 ...
 function changestatus(cmnt_id,status)
   { 
   	    var res = $.ajax(
	        {						
			   type: 'POST',
			   url: '<?php echo site_url('cocktail/changecocktailstatus/')?>',
			   dataType: 'post', 
	...
	function Editcocktail(id,cocktail_id)
{
		...
		var $modal = $('#ajax-modal');
		 ...
		 //alert(this.href);
		 var url='<?php echo base_url(); ?>cocktail/Editcocktailcomment/'+id+'/'+cocktail_id+'/';
		// return false;
...
<div class="page_content">
			<div class="container_fluid">
				<div class="row_fluid"> 
					...
					<div class="fl_right">	
								<?php echo anchor('cocktail','Back', 'class="btn blue  fl_left mar_r_5" '); ?>
					         </div>
					         <div class="clearfix"></div>
				</div> <div class="clearfix"></div>
				<?php if($msg && $msg=='success'){?>
 <div class="success_msg"><?php echo '<p>Comment edited successfully .</p>';?></div>
			...
			<?php if($one_cocktail_detail["cocktail_image"]!='' && file_exists(base_path().'upload/cocktail_thumb/'.$one_cocktail_detail["cocktail_image"])){?>
								...
								if($reply)
								{
								  ...
								  <?php }?>
									...
									<div class="media-body">
										<h4 class="media-heading"><?php echo $re->comment_title; ?> <span><?php echo getDuration($re->date_added); ?> /
											<a onclick="Editcocktail('<?php echo $re->cocktail_comment_id;?>','<?php echo $one_cocktail_detail["cocktail_id"]?>')" href="javascript://">Edit   </a> / 
											<a href="javascript:void(0);" onclick="return conform_delete('<?php echo $one_cocktail_detail["cocktail_id"] ?>','<?php echo $re->cocktail_comment_id ?>')">Delete</a>
											<input type="radio" id="status_<?php echo $re->cocktail_comment_id ?>" name="status_<?php echo $re->cocktail_comment_id ?>" onclick="changestatus('<?php echo $re->cocktail_comment_id ?>','active')"  value="active" <?php echo $re->status=='active'? 'checked=checked':'';?> />Show
											<input type="radio" id="status_<?php echo $re->cocktail_comment_id ?>" name="status_<?php echo $re->cocktail_comment_id ?>" onclick="changestatus('<?php echo $re->cocktail_comment_id ?>','inactive')" value="inactive"  <?php echo $re->status=='inactive'? 'checked=checked':'';?> />Hide</span>
											</span></h4>
										<p>
											<?php echo $re->comment; ?>
										 </p>
								  ...
								  <?php }?>
											...
											<div class="media-body">
												<h4 class="media-heading"><?php echo $sb->comment_title; ?>  <span><?php echo getDuration($re->date_added); ?> / 
													<a onclick="Editcocktail('<?php echo $sb->cocktail_comment_id;?>','<?php echo $one_cocktail_detail["cocktail_id"]?>')" href="javascript://">Edit   </a> /
													<a href="javascript:void(0);" onclick="return conform_delete('<?php echo $one_cocktail_detail["cocktail_id"] ?>','<?php echo $sb->cocktail_comment_id ?>')">Delete  </a></h4>
												<p>
											<?php echo $sb->comment; ?>
										 </p>
											</div>
							  ...
							  <?php }
								...
								<?php
									...
									echo form_open_multipart('cocktail/view_cocktail_comment/'.$cocktail_id,$attributes);
								?>	
