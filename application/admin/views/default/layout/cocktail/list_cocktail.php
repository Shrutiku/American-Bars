...
$(document).ready(function(){
	...
	function delete_rec(id,redirectpage,option,keyword,limit,offset)
	{
		...
		if(ans)
		{
			location.href = "<?php echo base_url(); ?>cocktail/delete_cocktail/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+limit+"/"+offset;
		}else{
			return false;
		}
	}
	...
	function getlimit(limit)
	{
		...
		{
			window.location.href='<?php echo base_url();?>cocktail/list_cocktail/'+limit+'/<?php echo $bars_id ?>';
		}
	...
	function getsearchlimit(limit)
	{
		...
		else
		{
			...
			window.location.href='<?php echo base_url();?>cocktail/search_list_cocktail/'+limit+'/<?php echo $bars_id.'/'.$option.'/'.$keyword; ?>';
		}
	...
	function gomain(x)
	{
		...
		if(x == 'all')
		{
			window.location.href= '<?php echo base_url();?>cocktail/list_cocktail/<?php echo $bars_id ?>';
		}
...
<div class="page_content">
			...
			<div class="container_fluid">
				...
				<div class="row_fluid"> 
					<h3 class="page_title">Cocktail Directory</h3>
				...
				<div class="row_fluid"> 
					<div class="portlet blue">
						<div class="portlet-title">
							<div class="caption fl_left">Cocktail  Directory</div>
							<div class="fl_right">
					...
					echo form_open('cocktail/search_list_cocktail/'.$limit,$attributes);?>
					<div class="fl_left">
					...
					<input type="hidden" name="bars_id" id="bars_id" value="<?php echo $bars_id; ?>" />
                            ...
                            <input type="button" name="refresh" id="refresh" value="Refresh" class="btn blue  fl_left mar10" onclick="document.location.href = '<?php echo site_url("cocktail/list_cocktail/20/".$bars_id); ?>'" /> 
								</form>
								</div>
					<div class="fl_right">	
						<?php  if($bars_id!='' && $bars_id!=0){?>
					<?php echo anchor('cocktail/add_newcocktail/'.$bars_id,'Add Cocktail', 'class="btn blue  fl_left mar_r_5" id="addbeer"'); ?>
				<?php } else {?>										
					<?php echo anchor('cocktail/add_cocktail/'.$bars_id,'Add New', 'class="btn blue  fl_left mar_r_5" id="addcocktail"'); ?>
					...
					<?php } ?>	
					<?php echo anchor('cocktail/import','Import xls', 'class="btn blue  fl_left mar_r_5" id="addbeer"'); ?>
					<a href="<?php echo front_base_url().'/upload/cocktail.xls';?>" class='btn purple fl_left mar_r_5'>Demo xls</a>
					...
					<div class="clear"></div>
									...
									<div class="clear"></div>
								<form name="frm_listlogin" id="frm_listlogin" action="<?php echo base_url();?>cocktail/action_cocktail" method="post">
						...
						?>
														...
														<?php  if($bars_id=='' || $bars_id==0){?>
														<?php echo  anchor('cocktail/edit_cocktail/'.$row->cocktail_id.'/'.$redirect_page.'/'.$option.'/'.$keyword.'/'.$limit.'/'.$offset.'/'.$bars_id,'<i class="comon_icon edit_icon"></i>','class="table_icon btn blue" id="cocktail_'.$row->cocktail_id.'" title="Edit cocktail"');  ?>
														<?php } ?>
