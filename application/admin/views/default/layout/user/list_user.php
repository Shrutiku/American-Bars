...
$(document).ready(function(){
	...
	function delete_rec(id,redirectpage,option,keyword,limit,offset,state)
	{
		...
		if(ans)
		{
			location.href = "<?php echo base_url(); ?>user/delete_user/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+limit+"/"+offset+"/"+state;
		}else{
			return false;
		}
	}
	...
	function getlimit(limit)
	{
		...
		{
			window.location.href='<?php echo base_url();?>user/list_enthusiast_user/<?php echo $state; ?>/'+limit+'/';
		}
	...
	function getsearchlimit(limit)
	{
		...
		else
		{
			...
			window.location.href='<?php echo base_url();?>user/search_list_user/<?php echo $state; ?>/'+limit+'/<?php echo $option.'/'.$keyword; ?>';
		}
...
<?php $att=array('id'=>'downloadCSV','name'=>'downloadCSV','class'=>'no-margin');
                                        echo form_open('user/euser_download/'.$state,$att) ?>
...
<div class="page_content">
			...
			<div class="container_fluid">
				...
				<div class="row_fluid"> 
					<h3 class="page_title">Enthusiast User listing</h3>
				...
				<div class="row_fluid"> 
					<div class="portlet blue">
						<div class="portlet-title">
							<div class="caption fl_left">Enthusiast User listing</div>
							<div class="fl_right">
					...
					echo form_open('user/search_list_user/'.$state."/".$limit,$attributes);?>
					<div class="fl_left">
					<div class="sdrop fl_left wid228">
					<span class="sspan fl_left ">Search By</span>
                            <select name="option" id="option" onchange="gomain(this.value)"  class="m_wrap fl_left mar0 wid-70" style="padding:6px;">
                            
								<option value="">--Please Select--</option>
                                <option value="full_name" <?php if($option=='full_name'){?> selected="selected"<?php }?>>First Name/Last Name</option>  
							...
							<input type="button" name="refresh" id="submit" value="Refresh" class="btn blue  fl_left mar10" onclick="document.location.href ='<?php echo site_url("user/list_enthusiast_user/".$state); ?>'" />
								</form>
								</div>
									<div class="fl_right">
					...
					<?php echo anchor('user/add_user/'.$limit.'/'.$offset,'Add New', 'class="btn blue  fl_left mar_r_5" id="adduser"'); ?>
					...
					<a href="javascript:void(0)" onclick="setaction('chk[]','delete', 'Are you sure, you want to delete selected record(s)?', 'frm_listlogin');" class="mar_r_5 btn black  fl_left" >Delete</a>
					...
					<div class="clear"></div>		
									...
									<div class="clear"></div>
			<form name="frm_listlogin" id="frm_listlogin" action="<?php echo base_url();?>user/action_user" method="post">
	<input type="hidden" name="offset" id="offset" value="<?php echo $offset; ?>" />
				   ...
				   <input type="hidden" name="redirect_page" id="redirect_page" value="<?php echo $redirect_page;?>"/>
							<div class="scroll-pane horizontal-only" >		<table class="table border" >
												...
												<th class="sorting_disabled" style="width: 6%;">Bar Fly Nickname</th>
												...
												<!--<th class="sorting" style="width: 5%;">User Right Upload Video</th>-->
												...
												<th class="sorting" style="width: 6%;">Album</th>
								...
								if($result)
								{
									...
									<td  style="width: 30px;"><input type="checkbox" name="chk[]" value="<?php echo $row->user_id ?>" class="chk"  /></td>
														...
														<td class=" sorting_1"><?php echo $row->nick_name; ?></td>
														<td><?php echo $row->email; ?></td>
														
														<td><?php echo date($site_setting->date_format .' h:i:s',strtotime($row->sign_up_date)); ?></td>
														<td>
														...
														<?php 
							//FUTURE 
							// 	echo anchor('user/edit_user/'.$row->user_id.'/'.$redirect_page.'/'.$option.'/'.$keyword.'/'.$limit.'/'.$offset,'<i class="comon_icon edit_icon"></i>','class="table_icon btn blue" id="user_'.$row->user_id.'" title="Edit user"'); 
						?>
														<!--Futture enhancement															
															<a class="btn green table_icon" href="javascript:;"><i class="comon_icon view_icon"></i></a>-->
														<?php echo  anchor('user/edit_user/'.$state."/".$row->user_id.'/'.$redirect_page.'/'.$option.'/'.$keyword.'/'.$limit.'/'.$offset,'<i class="comon_icon edit_icon"></i>','class="table_icon btn blue" id="user_'.$row->user_id.'" title="Edit user"');  ?>
													...
													<?php if($row->user_type=='user'){?>	
														<td align="center"  style="width: 1%;">
															<?php echo  anchor('album/list_album/20/'.$row->user_id,'Album','class="mini btn red" id="user_'.$row->user_id.'" title="Albums"');  ?>
														</td>
													<?php } else {?>
														<td align="center"  style="width: 1%;">
