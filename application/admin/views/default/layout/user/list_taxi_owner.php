...
$(document).ready(function(){
	...
	{
		var ans = confirm("Are you sure, you want to delete taxi owner?");
		if(ans)
		{
			location.href = "<?php echo base_url(); ?>taxi_owner/delete_taxi_owner/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+limit+"/"+offset+"/"+state;
		}else{
			return false;
		}
	}
	...
	function getlimit(limit)
	{
		...
		{
			window.location.href='<?php echo base_url();?>taxi_owner/list_taxi_owner/<?php echo $state; ?>/'+limit+'/';
		}
	...
	function getsearchlimit(limit)
	{
		...
		else
		{
			...
			window.location.href='<?php echo base_url();?>taxi_owner/search_list_taxi_owner/<?php echo $state; ?>/'+limit+'/<?php echo $option.'/'.$keyword; ?>';
		}
	...
	function gomain(x)
	{
		...
		if(x == 'all')
		{
			window.location.href= '<?php echo base_url();?>taxi_owner/list_taxi_owner/';
		}
...
<?php $att=array('id'=>'downloadCSV','name'=>'downloadCSV','class'=>'no-margin');
                                        echo form_open('user/taxi_user_download/'.$state,$att) ?>
...
<div class="page_content">
			...
			<div class="container_fluid">
				...
				<div class="row_fluid"> 
					<h3 class="page_title">List Taxi Owner</h3>
				...
				<div class="row_fluid"> 
					<div class="portlet blue">
						<div class="portlet-title">
							<div class="caption fl_left">Taxi Owner listing</div>
							<div class="fl_right">
					...
					echo form_open('taxi_owner/search_list_taxi_owner/'.$state."/".$limit,$attributes);?>
					<div class="fl_left">
					...
					<span class="sspan fl_left ">&nbsp;&nbsp;&nbsp;Search By</span>&nbsp;
							...
							<input type="button" name="refresh" id="submit" value="Refresh" class="btn blue  fl_left mar10" onclick="document.location.href ='<?php echo site_url("taxi_owner/list_taxi_owner/"); ?>'" />
								</form>
								</div>
									<div class="fl_right">
					...
					<?php echo anchor('taxi_owner/add_taxi_owner/'.$limit.'/'.$offset,'Add New', 'class="btn blue  fl_left mar_r_5" id="addtaxi_owner"'); ?>
					...
					<div class="clear"></div>		
									...
									<div class="clear"></div>
			<form name="frm_listlogin" id="frm_listlogin" action="<?php echo base_url();?>taxi_owner/action_taxi_owner" method="post">
	<input type="hidden" name="offset" id="offset" value="<?php echo $offset; ?>" />
				   ...
				   <input type="hidden" name="redirect_page" id="redirect_page" value="<?php echo $redirect_page;?>"/>
							<div class="scroll-pane horizontal-only" >		<table class="table border" >
												...
												<th class="sorting_disabled" style="width: 6%;">Company Name</th>
												<th class="sorting" style="width: 5%;">Email</th>
											    
												<!--<th class="sorting" style="width: 5%;">taxi_owner Right Upload Video</th>-->
								...
								if($result)
								{
									...
									<td  style="width: 30px;"><input type="checkbox" name="chk[]" value="<?php echo $row->user_id ?>" class="chk"  /></td>
														...
														<td><?php echo $row->taxi_company; ?></td>
														...
														<?php 
							//FUTURE 
							// 	echo anchor('taxi_owner/edit_taxi_owner/'.$row->user_id.'/'.$redirect_page.'/'.$option.'/'.$keyword.'/'.$limit.'/'.$offset,'<i class="comon_icon edit_icon"></i>','class="table_icon btn blue" id="taxi_owner_'.$row->user_id.'" title="Edit taxi_owner"'); 
						?>
														<!--Futture enhancement															
															<a class="btn green table_icon" href="javascript:;"><i class="comon_icon view_icon"></i></a>-->
														<?php echo  anchor('taxi_owner/edit_taxi_owner/'.$state."/".$row->user_id.'/'.$redirect_page.'/'.$option.'/'.$keyword.'/'.$limit.'/'.$offset,'<i class="comon_icon edit_icon"></i>','class="table_icon btn blue" id="taxi_owner_'.$row->user_id.'" title="Edit taxi_owner"');  ?>
