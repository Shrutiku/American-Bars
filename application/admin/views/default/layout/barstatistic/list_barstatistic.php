...
$(document).ready(function(){
	...
	{
		var ans = confirm("Are you sure, you want to delete barstatistic?");
		if(ans)
		{
			location.href = "<?php echo base_url(); ?>barstatistic/delete_barstatistic/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+limit+"/"+offset;
		}else{
			return false;
		}
	}
	...
	function getlimit(limit)
	{
		...
		{
			window.location.href='<?php echo base_url();?>barstatistic/list_barstatistic/'+limit+'/';
		}
	...
	function getsearchlimit(limit)
	{
		...
		else
		{
			...
			window.location.href='<?php echo base_url();?>barstatistic/search_list_barstatistic/'+limit+'/<?php echo $option.'/'.$keyword; ?>';
		}
	...
	function gomain(x)
	{
		...
		if(x == 'all')
		{
			window.location.href= '<?php echo base_url();?>barstatistic/list_barstatistic/';
		}
...
<div class="page_content">
			...
			<div class="container_fluid">
				...
				<div class="row_fluid"> 
					<h3 class="page_title">Bar Statistic</h3>
				...
				<div class="row_fluid"> 
					<div class="portlet blue">
						<div class="portlet-title">
							<div class="caption fl_left"> Bar Statistic </div>
							<div class="fl_right">
					...
					echo form_open('barstatistic/search_list_barstatistic/'.$limit,$attributes);?>
					<div class="fl_left">
					...
					<span class="sspan fl_left ">&nbsp;&nbsp;&nbsp;Search By</span>&nbsp;
                            <select name="option" id="option" onchange="gomain(this.value)"  class="m_wrap fl_left mar0" style="padding:6px;">
                                ...
                                <option value="question" <?php if($option=='question'){?> selected="selected"<?php }?>>Question</option>
								<!-- <option value="category" <?php if($option=='category'){?> selected="selected"<?php }?>>Category</option>
								<option value="answer" <?php if($option=='answer'){?> selected="selected"<?php }?>>Answer</option> -->
<?php /*?>                                <option value="type" <?php if($option=='type'){?> selected="selected"<?php }?>>Type</option>                 
                            ...
                            <input type="submit" name="submit" id="submit" value="Search" class="btn blue  fl_left mar10" /> 
                             ...
                             <input type="button" name="refresh" id="refresh" value="Refresh" class="btn blue  fl_left mar10" onclick="document.location.href = '<?php echo site_url("barstatistic"); ?>'" /> 
								</form>
								</div>
									<div class="fl_right">
					...
					<?php echo anchor('barstatistic/add_barstatistic','Add New', 'class="btn blue  fl_left mar_r_5" id="addbarstatistic"'); ?>
					...
					<div class="clear"></div>		
									...
									<div class="clear"></div>
					<form name="frm_listlogin" id="frm_listlogin" action="<?php echo base_url();?>barstatistic/action_barstatistic" method="post">
				   ...
				   <input type="hidden" name="redirect_page" id="redirect_page" value="<?php echo $redirect_page;?>"/>
								<div class="scroll-pane horizontal-only" >	<table class="table border" >
												...
												<th class="sorting_disabled" style="width: 6%;">Question</th>
												<th class="sorting" style="width: 5%;">Category</th>
												<!-- <th class="sorting" style="width: 5%;">Answer</th> -->
								...
								if($result)
								{
									...
									<td  style="width: 30px;"><input type="checkbox" name="chk[]" value="<?php echo $row->bar_statistic_id ?>" class="chk"  /></td>
														...
														<?php 
							//FUTURE 
							// 	echo anchor('barstatistic/edit_barstatistic/'.$row->bar_statistic_id.'/'.$redirect_page.'/'.$option.'/'.$keyword.'/'.$limit.'/'.$offset,'<i class="comon_icon edit_icon"></i>','class="table_icon btn blue" id="barstatistic_'.$row->bar_statistic_id.'" title="Edit barstatistic"'); 
						?>
														<!--Futture enhancement															
															<a class="btn green table_icon" href="javascript:;"><i class="comon_icon view_icon"></i></a>-->
														<?php echo  anchor('barstatistic/edit_barstatistic/'.$row->bar_statistic_id.'/'.$redirect_page.'/'.$option.'/'.$keyword.'/'.$limit.'/'.$offset,'<i class="comon_icon edit_icon"></i>','class="table_icon btn blue" id="barstatistic_'.$row->bar_statistic_id.'" title="Edit barstatistic"');  ?>
														<a class="table_icon btn red" href="javascript://" onClick="delete_rec('<?php echo $row->bar_statistic_id; ?>','<?php echo $redirect_page;?>','<?php echo $option?>','<?php echo $keyword?>','<?php echo $limit?>','<?php echo $offset; ?>')" title="Delete"><i class="comon_icon delete_icon"></i></a>
														</td>
													</tr>
								<?php $i++;} }else{ ?>
