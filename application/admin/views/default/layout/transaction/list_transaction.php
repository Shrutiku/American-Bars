...
<script type="text/javascript" language="javascript">
	...
	{
		var ans = confirm("Are you sure, you want to delete Transaction?");
		if(ans)
		{
			location.href = "<?php echo base_url(); ?>transaction/delete_transaction/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+from_date+"/"+to_date+"/"+limit+"/"+offset;
		}else{
			return false;
		}
	}
	...
	function getlimit(limit)
	{
		...
		{
			window.location.href='<?php echo base_url();?>transaction/list_transaction/'+limit+'/';
		}
	...
	function getsearchlimit(limit)
	{
		...
		else
		{
			...
			window.location.href='<?php echo base_url();?>transaction/search_list_transaction/'+limit+'/<?php echo $option.'/'.$keyword.'/'.@$from_date.'/'.@$to_date; ?>';
		}
	...
	function gomain(x)
	{
		...
		if(x == 'all')
		{
			window.location.href= '<?php echo base_url();?>transaction/list_transaction';
		}
...
<div class="page_content">
			...
			<div class="container_fluid">
				...
				<div class="row_fluid"> 
					<h3 class="page_title">Product Order Payment report</h3>
    ...
    <?php } ?>
				<div class="row_fluid"> 
					...
					<div class="clear"></div>
							...
							<?php $att=array('id'=>'downloadCSV','name'=>'downloadCSV','class'=>'no-margin');
										echo form_open('transaction/downloadPaymentRecord',$att) ?>
					...
					echo form_open('transaction/search_list_transaction/'.$limit,$attributes);?>
					<div class="fl_left">
					...
					<span class="sspan fl_left ">&nbsp;&nbsp;&nbsp;Search By</span>&nbsp;
						...
						<input class="search_key mar0 date" size="16" type="text" id="ui_date_picker_range_to" name="to_date" value="<?php echo @$to_date=='1V1' ? '':@$to_date;?>" placeholder="To"/>
                            ...
                            <input type="submit" name="submit" id="submit" style="margin-right: 6px;" value="Search" class="btn blue   fl_left mar0" />
                             <input type="button" name="refresh" id="refresh" value="Refresh" class="btn blue  fl_left mar10" onclick="document.location.href = '<?php echo site_url("transaction/list_transaction"); ?>'" /> 
								</form>
								</div>
									<div class="fl_right">
			...
			<a href="javascript://" onclick="donwloadCSV()" class="btn blue  fl_left mar_r_5 btn black"><i class="icon-download"></i> download</a>							
					<!--<?php echo anchor('transaction/add_transaction','Add New', 'class="btn blue  fl_left mar_r_5" id="addtransaction"'); ?>-->
					...
					<div class="clear"></div>		
									...
									<div class="clear"></div>
			<form name="frm_listlogin" id="frm_listlogin" action="<?php echo base_url();?>transaction/action_transaction" method="post">
	<input type="hidden" name="offset" id="offset" value="<?php echo $offset; ?>" />
				   ...
				   <input type="hidden" name="redirect_page" id="redirect_page" value="<?php echo $redirect_page;?>"/>
								...
								<div class="scroll-pane horizontal-only" >	<table class="table border" >
												...
												<th class="sorting" style="width: 18%;">Price</th>
												<th class="sorting" style="width: 18%;">Date</th>
								...
								if($result)
								{
									...
									<td  style="width: 30px;"><input type="checkbox" name="chk[]" value="<?php echo $row->transaction_id ?>" class="chk"  /></td>
														<td class=" sorting_1"><?php echo $row->txn_id; ?></td>
														<td> <?php if($row->user_id !='0'){echo $row->first_name .' '. $row->last_name ; } else { echo "----------"; } ?> </td>
														...
														<td class="center"> <?php echo ucfirst($row->transaction_status); ?> </span></td>
														...
														?>
														 <a target="_blank" title="View" class="table_icon btn blue" href="<?php echo site_url('order/orderDetails/'.$row->order_id); ?>" ><i class="comon_icon view_icon"></i></a>
