...
<script type="text/javascript" language="javascript">
	...
	{
		var ans = confirm("Are you sure, you want to delete Bar ?");
		if(ans)
		{
			location.href = "<?php echo base_url(); ?>archived_bar/delete_archived_bar/<?php echo $bar_type; ?>/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+limit+"/"+offset;
		}else{
			return false;
		}
	}
	...
	function getlimit(limit)
	{
		...
		{
			window.location.href='<?php echo base_url();?>archived_bar/list_archived_bar/<?php echo $bar_type;?>/'+limit+'/';
		}
	...
	function getsearchlimit(limit)
	{
		...
		else
		{
			...
			window.location.href='<?php echo base_url();?>archived_bar/search_list_archived_bar/<?php echo $bar_type; ?>/'+limit+'/<?php echo $option.'/'.$keyword; ?>';
		}
	...
	function gomain(x)
	{
		...
		if(x == 'all')
		{
			window.location.href= '<?php echo base_url();?>archived_bar/list_archived_bar/<?php echo $bar_type; ?>';
		}
...
function setaction(elename, actionval, actionmsg, formname) {
	...
	function reply_message(id,redirectpage,option,keyword,limit){
		location.href = "<?php echo base_url(); ?>archived_bar/comment/<?php echo $bar_type; ?>/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+limit;
	}
...
function changePassword(id,limit,offset)
{
		...
		var $modal = $('#ajax-modal');
		 ...
		 //alert(this.href);
		 var url='<?php echo base_url(); ?>archived_bar/setNewPassword/'+id+'/';
		// return false;
		  setTimeout(function(){
...
function donwloadCSV(){
 ...
 <?php $att=array('id'=>'downloadCSV','name'=>'downloadCSV','class'=>'no-margin');
                                        echo form_open('archived_bar/download',$att) ?>
...
<div class="page_content">
			...
			<div class="container_fluid">
				...
				<div class="row_fluid"> 
					<h3 class="page_title"><?php //echo $bar_type=='full_mug' ? 'Full Mug':'Half Mug'; ?> Archived Bar Listing</h3>
   ...
   if($er!='')
    ...
    <?php } ?>
				<div class="row_fluid"> 
					...
					echo form_open('archived_bar/search_list_archived_bar/'.$bar_type."/".$limit,$attributes);?>
					<div class="fl_left">
					<div class="sdrop fl_left wid140">
                            ...
                            <input type="submit" name="submit" id="submit" value="Search" class="btn blue  fl_left mar10" />
                              <input type="button" name="refresh" id="submit" value="Refresh" class="btn blue  fl_left mar10" onclick="document.location.href = '<?php echo site_url("archived_bar/list_archived_bar/".$bar_type); ?>'" /> 
								</form>
								</div>
									<div class="fl_right">
					...
					<div class="clear"></div>		
									...
									<div class="clear"></div>
			<form name="frm_listlogin" id="frm_listlogin" action="<?php echo base_url();?>archived_bar/action_archived_bar/<?php echo $bar_type; ?>" method="post">
	<input type="hidden" name="offset" id="offset" value="<?php echo $offset; ?>" />
				   ...
				   <div class="scroll-pane horizontal-only" >
									<table class="table border" >
												...
												<th class="sorting" style="width: 10%;">Reviews And Ratings</th><?php if($bar_type=='full_mug'){?>
												<?php } ?>
								...
								if($result)
								{
									...
									<td  style="width: 30px;"><input type="checkbox" name="chk[]" value="<?php echo $row->bar_id ?>" class="chk"  /></td>
														...
														<?php $getbar = $this->archived_bar_model->get_one_bar($row->bar_id);?>
