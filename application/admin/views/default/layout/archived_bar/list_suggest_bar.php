<script type="text/javascript" language="javascript">
	...
	{
		var ans = confirm("Are you sure, you want to delete suggest bar?");
		if(ans)
		{
			location.href = "<?php echo base_url(); ?>suggest_bar/delete_suggest_bar/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+limit+"/"+offset;
		}else{
			return false;
		}
	}
	...
	function getlimit(limit)
	{
		...
		{
			window.location.href='<?php echo base_url();?>suggest_bar/list_suggest_bar/'+limit+'/';
		}
	...
	function getsearchlimit(limit)
	{
		...
		else
		{
			...
			window.location.href='<?php echo base_url();?>suggest_bar/search_list_suggest_bar/'+limit+'/<?php echo $option.'/'.$keyword; ?>';
		}
	...
	function gomain(x)
	{
		...
		if(x == 'all')
		{
			window.location.href= '<?php echo base_url();?>suggest_bar/list_suggest_bar/';
		}
...
<div class="page_content">
			...
			<div class="container_fluid">
				...
				<div class="row_fluid"> 
					<h3 class="page_title">Suggest Bar List </h3>
		...
		if($msg != ""){
	     if($msg == "insert"){ $error = ADD_NEW_RECORD;}
			...
			if($msg == "approve"){ $error = 'Bar Approve Successfully.';}
    ?>
				...
				<div class="row_fluid"> 
					<div class="portlet blue">
						<div class="portlet-title">
							<div class="caption fl_left"> Suggest Bar List </div>
							<div class="fl_right">
					...
					echo form_open('suggest_bar/search_list_suggest_bar/'.$limit,$attributes);?>
					<div class="fl_left">
					...
					<span class="sspan fl_left ">&nbsp;&nbsp;&nbsp;Search By</span>&nbsp;
                            <select name="option" id="option" onchange="gomain(this.value)"  class="m_wrap fl_left mar0" style="padding:6px;">
								...
								<option value="name" <?php if($option=='name'){?> selected="selected"<?php }?>>Bar Name</option>  
<?php /*?>                                <option value="type" <?php if($option=='type'){?> selected="selected"<?php }?>>Type</option>                 
                            ...
                            <input type="submit" name="submit" id="submit" value="Search" class="btn blue  fl_left mar10" /> 
                             ...
                             <input type="button" name="refresh" id="refresh" value="Refresh" class="btn blue  fl_left mar10" onclick="document.location.href = '<?php echo site_url("suggest_bar/list_suggest_bar"); ?>'" /> 
								</form>
								</div>
									<div class="fl_right">
					...
					<a href="javascript:void(0)" onclick="setaction('chk[]','pending', 'Are you sure, you want to app selected record(s)?', 'frm_listlogin');" class="btn purple fl_left mar_r_5" >Approved</a>
				   ...
				   <input type="hidden" name="redirect_page" id="redirect_page" value="<?php echo $redirect_page;?>"/>
								<div class="scroll-pane horizontal-only" >	<table class="table border" >
												...
												<th class="sorting" style="width: 5%;">Zip Code</th>
												<th class="sorting" style="width: 5%;">Number Of Criteria</th>
								...
								if($result)
								{
									...
									<td  style="width: 30px;"><input type="checkbox" name="chk[]" value="<?php echo $row->suggest_bar_id; ?>" class="chk"  /></td>
													<td class=" sorting_1"><?php echo $row->bar_name; 
														...
														<td style="text-align:center;"><a class="table_icon btn blue" href="<?php echo site_url("suggest_bar/view_suggest_bar/".$row->suggest_bar_id); ?>"><i class="comon_icon view_icon"</a></td>
														<td><?php 
							
																echo anchor('suggest_bar/edit_suggest_bar/'.$row->suggest_bar_id.'/'.$redirect_page.'/'.$option.'/'.$keyword.'/'.$limit.'/'.$offset,'<i class="comon_icon edit_icon"></i>','class="table_icon btn blue"  title="Edit Bar"'); 
														?>
														
															<a class="table_icon btn red" href="javascript://" onClick="delete_rec('<?php echo $row->suggest_bar_id; ?>','<?php echo $redirect_page;?>','<?php echo $option?>','<?php echo $keyword?>','<?php echo $limit?>','<?php echo $offset; ?>')" title="Delete"><i class="comon_icon delete_icon"></i></a></td>
													</tr>
								<?php $i++;} }else{ ?>
