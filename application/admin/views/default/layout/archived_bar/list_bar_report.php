<script type="text/javascript" language="javascript">
	...
	function delete_rec(id,redirectpage,keyword,limit,offset)
	{
		var ans = confirm("Are you sure, you want to delete bar report?");
		if(ans)
		{
			location.href = "<?php echo base_url(); ?>report/delete_bar_report/"+id+"/"+redirectpage+"/"+keyword+"/"+limit+"/"+offset;
		}else{
			return false;
		}
	}
	...
	function getlimit(limit)
	{
		...
		{
			window.location.href='<?php echo base_url();?>report/list_bar_report/'+limit+'/';
		}
	...
	function getsearchlimit(limit)
	{
		...
		else
		{
			...
			window.location.href='<?php echo base_url();?>report/search_list_bar_report/'+limit+'/<?php echo $keyword; ?>';
		}
	...
	function gomain(x)
	{
		...
		if(x == 'all')
		{
			window.location.href= '<?php echo base_url();?>report/list_bar_report/';
		}
...
function setaction(elename, actionval, actionmsg, formname) {
	...
	function reply_message(id,redirectpage,option,keyword,limit,offset){
		location.href = "<?php echo base_url(); ?>report/view/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+limit+"/"+offset;
	}
...
<div class="page_content">
			...
			<div class="container_fluid">
				...
				<div class="row_fluid"> 
					<h3 class="page_title">Bar Report Listing</h3>
    ...
    <?php } ?>
				<div class="row_fluid"> 
					...
					echo form_open('report/search_list_bar_report/'.$limit,$attributes);?>
					<div class="fl_left">
					
							<input type="text" name="keyword" id="keyword" value="<?php echo $keyword_data;?>"  class="search_key mar0" placeholder="Search by Report Status, Bar Name" /> 
							</div>           
                            <input type="submit" name="submit" id="submit" value="Search" class="btn blue  fl_left mar10" />
                              <input type="button" name="refresh" id="submit" value="Refresh" class="btn blue  fl_left mar10" onclick="document.location.href = '<?php echo site_url("report/list_bar_report/"); ?>'" /> 
								</form>
								</div>
									<div class="fl_right">	
					...
					<div class="clear"></div>																					
									...
									<div class="clear"></div>
			<form name="frm_listlogin" id="frm_listlogin" action="<?php echo base_url();?>report/action_bar_report" method="post">
				   ...
				   <div class="scroll-pane horizontal-only" >
									...
									<table class="table border" >
												...
												<th class="sorting_disabled">Bar Report Status</th>
												<th class="sorting" >Bar Name</th>
								...
								if($result)
								{
									...
									<td  style="width: 30px;"><input type="checkbox" name="chk[]" value="<?php echo $row->report_id ?>" class="chk"  /></td>
										<td class=" sorting_1"><?php echo $row->status; if($row->r_st == '0')
														{?>
															<img src="<?php echo front_base_url(); ?>/upload/gif_new.gif" width="32" height="10" />
														<?php }
														?></td>
										<td class=" sorting_1"><a target="_blank" href="<?php echo front_base_url().'bar/details/'.$row->bar_slug; ?>"><?php echo $row->bar_title; ?></a></td>
										<td class=" sorting_1"><?php echo $row->reported_by; ?></td>
										<td><a class="table_icon btn red" href="javascript://" onClick="delete_rec('<?php echo $row->report_id;?>','<?php echo $redirect_page;?>','<?php echo $keyword?>','<?php echo $limit?>','<?php echo $offset; ?>')" title="Delete"><i class="comon_icon delete_icon"></i></a>
											
											<?php echo  anchor('report/viewreport/'.$row->report_id,'<i class="comon_icon view_icon"></i>','class="table_icon btn blue"  id="user_'.$row->report_id.'" title="View"');  ?>
										</td>
