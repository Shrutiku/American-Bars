<script type="text/javascript" language="javascript">
	...
	{
		var ans = confirm("Are you sure, you want to delete taxi company report?");
		if(ans)
		{
			location.href = "<?php echo base_url(); ?>report/delete_taxi_company_report/"+id+"/"+redirectpage+"/"+keyword+"/"+limit+"/"+offset;
		}else{
			return false;
		}
	}
	...
	function getlimit(limit)
	{
		...
		{
			window.location.href='<?php echo base_url();?>report/list_taxi_company_report/'+limit+'/';
		}
	...
	function getsearchlimit(limit)
	{
		...
		else
		{
			...
			window.location.href='<?php echo base_url();?>report/search_list_taxi_company_report/'+limit+'/<?php echo $keyword; ?>';
		}
	...
	function gomain(x)
	{
		...
		if(x == 'all')
		{
			window.location.href= '<?php echo base_url();?>report/list_taxi_company_report/';
		}
...
<div class="page_content">
			...
			<div class="container_fluid">
				...
				<div class="row_fluid"> 
					<h3 class="page_title">Taxi Company Report Listing</h3>
    ...
    <?php } ?>
				<div class="row_fluid"> 
					...
					echo form_open('report/search_list_taxi_company_report/'.$limit,$attributes);?>
					<div class="fl_left">
                            ...
                            <input type="submit" name="submit" id="submit" value="Search" class="btn blue  fl_left mar10" />
                              <input type="button" name="refresh" id="submit" value="Refresh" class="btn blue  fl_left mar10" onclick="document.location.href = '<?php echo site_url("report/list_taxi_company_report/"); ?>'" /> 
								</form>
								</div>
									<div class="fl_right">	
					...
					<div class="clear"></div>																					
									...
									<div class="clear"></div>
			<form name="frm_listlogin" id="frm_listlogin" action="<?php echo base_url();?>report/action_taxi_company_report" method="post">
				   ...
				   <div class="scroll-pane horizontal-only" >
									...
									<table class="table border" >
												...
												<th class="sorting_disabled">Taxi Company Report Status</th>
												<th class="sorting" >Taxi Company Name</th>
								...
								if($result)
								{
									...
									<td  style="width: 30px;"><input type="checkbox" name="chk[]" value="<?php echo $row->report_id ?>" class="chk"  /></td>
										...
										<td><a class="table_icon btn red" href="javascript://" onClick="delete_rec('<?php echo $row->report_id;?>','<?php echo $redirect_page;?>','<?php echo $keyword?>','<?php echo $limit?>','<?php echo $offset; ?>')" title="Delete"><i class="comon_icon delete_icon"></i></a>
											...
											<?php echo  anchor('report/view_taxi_company_report/'.$row->report_id,'<i class="comon_icon view_icon"></i>','class="table_icon btn blue"  id="user_'.$row->report_id.'" title="View"');  ?>
										</td>
