...
$(document).ready(function(){
	...
	function getlimit(limit)
	{
		...
		{
			window.location.href='<?php echo base_url();?>forum_suggestion/list_forum_suggestion/'+limit;
		}
	...
	function getsearchlimit(limit)
	{
		...
		else
		{
			...
			window.location.href='<?php echo base_url();?>forum_suggestion/search_list_forum_suggestion/'+limit+'/<?php echo $option.'/'.$keyword; ?>';
		}
	...
	function gomain(x)
	{
		...
		if(x == 'all')
		{
			window.location.href= '<?php echo base_url();?>forum_suggestion/list_forum_suggestion';
		}
...
<div class="page_content">
			...
			<div class="container_fluid">
				...
				<div class="row_fluid"> 
					<h3 class="page_title">Forum Suggestion</h3>
    ...
    <?php } ?>
				<div class="row_fluid"> 
					<div class="portlet blue">
						<div class="portlet-title">
							<div class="caption fl_left">Forum Suggestion</div>
							<div class="fl_right">
					...
					echo form_open('forum_suggestion/search_list_forum_suggestion/'.$limit,$attributes);?>
					<div class="fl_left">
					...
					<span class="sspan fl_left ">&nbsp;&nbsp;&nbsp;Search By</span>&nbsp;
                            <select name="option" id="option" onchange="gomain(this.value)"  class="m_wrap fl_left mar0" style="padding:6px;">
                                ...
                                <option value="forum_name" <?php if($option=='forum_name'){?> selected="selected"<?php }?>>Forum Name</option>  
                            </select>
					</div>
                            ...
                            <input type="submit" name="submit" id="submit" value="Search" class="btn blue  fl_left mar10" /> 
                             ...
                             <input type="button" name="refresh" id="refresh" value="Refresh" class="btn blue  fl_left mar10" onclick="document.location.href = '<?php echo site_url("forum_suggestion/list_forum_suggestion/20/"); ?>'" /> 
								</form>
								</div>
									<div class="fl_right">
				...
				<a href="javascript:void(0)" onclick="setaction('chk[]','active', 'Are you sure, you want to activate selected record(s)?', 'frm_listlogin');" class="btn purple fl_left mar_r_5" >Active</a>
					...
					<div class="clear"></div>		
									...
									<div class="clear"></div>
			<form name="frm_listlogin" id="frm_listlogin" action="<?php echo base_url();?>forum_suggestion/action_forum_suggestion" method="post">
	<input type="hidden" name="offset" id="offset" value="<?php echo $offset; ?>" />
				   ...
				   <input type="hidden" name="redirect_page" id="redirect_page" value="<?php echo $redirect_page;?>"/>
									<div class="scroll-pane horizontal-only" ><table class="table border" >
												...
												<th class="sorting_disabled" style="width: 6%;">Topic Name</th>
												<th class="sorting_disabled" style="width: 6%;">Category Name</th>
								...
								if($result)
								{
									...
									<td  style="width: 30px;"><input type="checkbox" name="chk[]" value="<?php echo $row->forum_id ?>" class="chk"  /></td>
														<td class="sorting_1"><?php echo $row->topic_name; if($row->states == 'unread')
														{?>
															<img src="<?php echo front_base_url(); ?>/upload/gif_new.gif" width="32" height="10" />
														<?php }
														?></td>
														<td class="sorting_1"><?php echo $row->forum_category_name; ?></td>
														<td class="center">
															
															<?php $cls = ($row->status=='pending')?'purple':'yellow';?>
															<span class="<?php echo $cls;?>"><?php echo ucfirst($row->status); ?></span>
														</td>
														<td class="center">
															<a class="table_icon btn blue" href="<?php echo site_url("forum_suggestion/view_forum/".$row->forum_id); ?>" title="View"><i class="comon_icon view_icon"></i></a>
														</td>
														<td style="text-align: center;">
														<?php 
							//FUTURE 
							// 	echo anchor('forum_suggestion/edit_forum_suggestion/'.$row->forum_suggestion_id.'/'.$redirect_page.'/'.$option.'/'.$keyword.'/'.$limit.'/'.$offset,'<i class="comon_icon edit_icon"></i>','class="table_icon btn blue" id="forum_suggestion_'.$row->forum_suggestion_id.'" title="Edit forum_suggestion"'); 
						?>
														<!--Futture enhancement															
															<a class="btn green table_icon" href="javascript:;"><i class="comon_icon view_icon"></i></a>-->
															<!-- <?php  if($bars_id=='' || $bars_id==0){?>
														<?php echo  anchor('forum_suggestion/edit_forum_suggestion/'.$row->forum_id.'/'.$redirect_page.'/'.$option.'/'.$keyword.'/'.$limit.'/'.$offset.'/'.$bars_id,'<i class="comon_icon edit_icon"></i>','class="table_icon btn blue" id="forum_suggestion_'.$row->forum_id.'" title="Edit forum_suggestion"');  ?>
														<?php } ?> -->
														<a class="table_icon btn red" href="javascript://" onClick="delete_rec('<?php echo $row->forum_id; ?>','<?php echo $redirect_page;?>','<?php echo $option?>','<?php echo $keyword?>','<?php echo $limit?>','<?php echo $offset; ?>')" title="Delete"><i class="comon_icon delete_icon"></i></a>
														</td>
													</tr>
								<?php $i++;} }else{ ?>
...
<script>
	function delete_rec(id,redirectpage,option,keyword,limit,offset)
	{
		...
		if(ans)
		{
			location.href = "<?php echo base_url(); ?>forum_suggestion/delete_forum_suggestion/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+limit+"/"+offset;
		}else{
			return false;
		}
	}
