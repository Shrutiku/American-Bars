<script type="text/javascript" language="javascript">
	function delete_rec(id,redirectpage,option,keyword,limit,offset) {
		var ans = confirm("Are you sure, you want to delete diet?");
		if(ans) {
			location.href = "<?php echo base_url(); ?>diet/delete_diet/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+limit+"/"+offset;
		}else{
			return false;
		}
	}
	...
	function getlimit(limit) {
		...
		if(limit=='0'){
			...
			window.location.href='<?php echo base_url();?>diet/list_diet/'+limit+'/';
		}
	}	
	...
	function getsearchlimit(limit) {
		if(limit=='0') {
			...
			window.location.href='<?php echo base_url();?>diet/search_list_diet/'+limit+'/<?php echo $option.'/'.$keyword; ?>';
		}
	...
	function gomain(x) {
		if(x == 'all')
		{
			window.location.href= '<?php echo base_url();?>diet/list_diet';
		}
	}
...
<div class="page_content">
			<div class="container_fluid">	
				<div class="row_fluid"> 
					<h3 class="page_title">List diet</h3>
				</div>
					<?php 
				...
				<div class="row_fluid"> 
					<div class="portlet blue">
						<div class="portlet-title">
							<div class="caption fl_left">Diet list</div>
							<div class="fl_right">
						...
						<div class="portlet-body form">
							...
							<div class="fl_left">
								 <?php			 
									...
									echo form_open('diet/search_list_diet/'.$limit,$attributes);?>
									<div class="fl_left">
										<div class="sdrop fl_left wid228">
											<span class="sspan fl_left ">&nbsp;&nbsp;&nbsp;Search By</span>&nbsp;
												<select name="option" id="option" onchange="gomain(this.value)"  class="m_wrap fl_left mar0" style="padding:6px;">
													...
													<option value="diet_name" <?php if($option=='diet_name'){?> selected="selected"<?php }?>>Diet Name</option>  
									...
									<?php echo anchor('diet/add_diet','Add New', 'class="btn blue  fl_left mar_r_5" id="adddiet"'); ?>
									...
									<div class="clear"></div>
								<form name="frm_listlogin" id="frm_listlogin" action="<?php echo base_url();?>diet/action_diet" method="post">
									...
									<table class="table border" >
										<thead>
											...
											<th class="sorting" style="width: 20%;">Diet Name</th>
										...
										<tbody role="alert" aria-live="polite" aria-relevant="all">	
											...
											if($result)
											{
												...
												<tr class="<?php echo $cls ?>">
													<td  style="width: 30px;"><input type="checkbox" name="chk[]" value="<?php echo $row->diet_id ?>" class="chk"  /></td>
													<td class=" sorting_1"><?php echo $row->diet_name;?></td>
													<td class="center">
														<?php $cls = ($row->status=='Active')?'purple':'yellow';?>
														<span class="<?php echo $cls;?>"><?php echo ucfirst($row->status); ?></span>
													</td>
													<td>
														<?php echo anchor('diet/edit_diet/'.$row->diet_id.'/'.$redirect_page.'/'.$option.'/'.$keyword.'/'.$limit.'/'.$offset,'<i class="comon_icon edit_icon"></i>','class="table_icon btn blue" id="diet_'.$row->diet_id.'" title="Edit diet"'); ?>
														<a class="table_icon btn red" href="javascript://" onClick="delete_rec('<?php echo $row->diet_id; ?>','<?php echo $redirect_page;?>','<?php echo $option?>','<?php echo $keyword?>','<?php echo $limit?>','<?php echo $offset; ?>')" title="Delete"><i class="comon_icon delete_icon"></i></a>
													</td>
												</tr>
											<?php $i++;} }else{ ?>															
												<tr class="odd">
													<td class=" sorting_1" colspan="4" style="text-align:center!important;">No Records Found</td>	
												</tr>
											<?php } ?>
