...
$(document).ready(function(){
	...
	{
		var ans = confirm("Are you sure, you want to delete resource?");
		if(ans)
		{
			location.href = "<?php echo base_url(); ?>resource/delete_resource/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+limit+"/"+offset;
		}else{
			return false;
		}
	}
	...
	function getlimit(limit)
	{
		...
		{
			window.location.href='<?php echo base_url();?>resource/list_resource/'+limit+'/';
		}
	...
	function getsearchlimit(limit)
	{
		...
		else
		{
			...
			window.location.href='<?php echo base_url();?>resource/search_list_resource/'+limit+'/<?php echo $option.'/'.$keyword; ?>';
		}
	...
	function gomain(x)
	{
		...
		if(x == 'all')
		{
			window.location.href= '<?php echo base_url();?>resource/list_resource/';
		}
...
<div class="page_content">
			...
			<div class="container_fluid">
				...
				<div class="row_fluid"> 
					<h3 class="page_title"> Resource </h3>
				...
				<div class="row_fluid"> 
					<div class="portlet blue">
						<div class="portlet-title">
							<div class="caption fl_left"> Resource </div>
							<div class="fl_right">
					...
					echo form_open('resource/search_list_resource/'.$limit,$attributes);?>
					<div class="fl_left">
					...
					<span class="sspan fl_left ">&nbsp;&nbsp;&nbsp;Search By</span>&nbsp;
                            <select name="option" id="option" onchange="gomain(this.value)"  class="m_wrap fl_left mar0" style="padding:6px;">                     
                                ...
                                <option value="resource_title" <?php if($option=='resource_title'){?> selected="selected"<?php }?>>Resource Title </option>
								<option value="resource_category" <?php if($option=='resource_category'){?> selected="selected"<?php }?>>Category</option>
                            </select>
					</div>
                            ...
                            <input type="submit" name="submit" id="submit" value="Search" class="btn blue  fl_left mar10" /> 
                             ...
                             <input type="button" name="refresh" id="refresh" value="Refresh" class="btn blue  fl_left mar10" onclick="document.location.href = '<?php echo site_url("resource"); ?>'" /> 
								</form>
								</div>
									<div class="fl_right">
					...
					<?php echo anchor('resource/add_resource','Add New', 'class="btn blue  fl_left mar_r_5" id="addresource"'); ?>
					...
					<div class="clear"></div>		
									...
									<div class="clear"></div>
					<form name="frm_listlogin" id="frm_listlogin" action="<?php echo base_url();?>resource/action_resource" method="post">
				   ...
				   <input type="hidden" name="redirect_page" id="redirect_page" value="<?php echo $redirect_page;?>"/>
							<div class="scroll-pane horizontal-only" >		<table class="table border" >
												...
												<th class="sorting_disabled" style="width: 6%;">Resource Title</th>
												<!-- <th class="sorting" style="width: 5%;">Description</th> -->
												<th class="sorting" style="width: 5%;">Category</th>
								...
								if($result)
								{
									...
									<td  style="width: 30px;"><input type="checkbox" name="chk[]" value="<?php echo $row->resource_id ?>" class="chk"  /></td>
														<td class=" sorting_1"><?php echo $row->resource_title; ?></td>
														<!-- <td class=" sorting_1"><?php echo $row->resource_desc; ?></td> -->
														<td><?php echo ucfirst(str_replace("_"," ",$row->resource_category)); ?></td>
														...
														<?php 
							//FUTURE 
							// 	echo anchor('resource/edit_resource/'.$row->resource_id.'/'.$redirect_page.'/'.$option.'/'.$keyword.'/'.$limit.'/'.$offset,'<i class="comon_icon edit_icon"></i>','class="table_icon btn blue" id="resource_'.$row->resource_id.'" title="Edit resource"'); 
						?>
														<!--Futture enhancement															
															<a class="btn green table_icon" href="javascript:;"><i class="comon_icon view_icon"></i></a>-->
														<?php echo  anchor('resource/edit_resource/'.$row->resource_id.'/'.$redirect_page.'/'.$option.'/'.$keyword.'/'.$limit.'/'.$offset,'<i class="comon_icon edit_icon"></i>','class="table_icon btn blue" id="resource_'.$row->resource_id.'" title="Edit resource"');  ?>
														<a class="table_icon btn red" href="javascript://" onClick="delete_rec('<?php echo $row->resource_id; ?>','<?php echo $redirect_page;?>','<?php echo $option?>','<?php echo $keyword?>','<?php echo $limit?>','<?php echo $offset; ?>')" title="Delete"><i class="comon_icon delete_icon"></i></a>
														</td>
													</tr>
								<?php $i++;} }else{ ?>
