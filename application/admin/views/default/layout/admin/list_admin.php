<script type="text/javascript" language="javascript">
	...
	{
		var ans = confirm("Are you sure, you want to delete Administrator?");
		if(ans)
		{
			location.href = "<?php echo base_url(); ?>admin/delete_admin/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+limit+"/"+offset;
		}else{
			return false;
		}
	}
...
<div class="page_content">
			...
			<div class="container_fluid">
				...
				<div class="row_fluid"> 
					<h3 class="page_title">Admin Listing</h3>
    ...
    <?php } ?>
		...
		<input type="button" name="refresh" id="submit" value="Refresh" class="btn blue  fl_left mar10" onclick="document.location.href ='<?php echo site_url("admin/list_admin/"); ?>'" />							
								</form>
								</div>
								<?php } ?>
									<div class="fl_right">
				...
				<?php if($this->session->userdata('admin_type')=='1'){?>						
					<?php echo anchor('admin/add_admin','Add New', 'class="btn blue  fl_left mar_r_5" id="addadmin"'); ?>
	...
	<input type="hidden" name="offset" id="offset" value="<?php echo $offset; ?>" />
				   ...
				   <div class="scroll-pane horizontal-only" >
									<table class="table border" >
												...
												<th class="sorting_disabled" style="width: 6%;"><?php echo $this->lang->line("admin.user_name");?></th>
												<th class="sorting" style="width: 12%;"><?php echo $this->lang->line("admin.full_name");?></th>
												<th class="sorting" style="width: 18%;"><?php echo $this->lang->line("admin.email");?></th>
												<th class="sorting" style="width: 5%;"><?php echo $this->lang->line("admin.status");?></th>
											 <th class="sorting" style="width: 5%;">Rights</th>
												<!--	<th class="sorting" style="width: 1%;">Logs</th> -->
												<th class="sorting" style="width: 6%;"><?php echo $this->lang->line("admin.action");?></th>
												</tr>
								...
								if($result)
								{
									...
									{    $cls=($i%2==0)?'even':'odd';
									 ?>
							<?php if($row->admin_id==get_authenticateadminID() || $this->session->userdata('admin_type')==1){?>		 
									<tr class="<?php echo $cls ?>">
									<td  style="width: 30px;"><input type="checkbox" name="chk[]" value="<?php echo $row->admin_id ?>" class="chk"  /></td>
														<td class=" sorting_1"><?php echo ($row->username!='')?$row->username:'N/A'; ?></td>
														<td><?php echo $row->first_name.' '.$row->last_name; ?></td>
														<td><?php echo $row->email ; ?></td>
														<td class="center">
															<?php $cls = ($row->active=='Active')?'purple':'yellow';?>
															<span class="<?php echo $cls;?>"><?php echo ucfirst($row->active); ?></span>
														</td>
														
														 <td class="center">
														 	
														 	<?php if($row->admin_type!=$this->session->userdata('admin_type')) { if($this->session->userdata('admin_type')=='1'){echo anchor('rights/assign_rights/'.$row->admin_id.'/'.$limit.'/'.$offset,'<i class="halflings-icon ok"></i>'); }else{ echo "N/A";} } ?>
														 	
														 	</td>
													<!--	<td>
														<a class="table_icon btn green" href="<?php echo site_url('admin/list_logactivity/'.$row->admin_id);?>">
														...
														<?php 
																...
																echo anchor('admin/edit_admin/'.$row->admin_id.'/'.$redirect_page.'/'.$option.'/'.$keyword.'/'.$limit.'/'.$offset,'<i class="comon_icon edit_icon"></i>','class="table_icon btn blue" id="admin_'.$row->admin_id.'" title="Edit Admin"'); 
														?>
													<?php if($this->session->userdata('admin_type')==1 && $row->admin_id!=get_authenticateadminID()){ ?>	
														<a class="table_icon btn red" href="javascript://" onClick="delete_rec('<?php echo $row->admin_id; ?>','<?php echo $redirect_page;?>','<?php echo $option?>','<?php echo $keyword?>','<?php echo $limit?>','<?php echo $offset; ?>')" title="Delete"><i class="comon_icon delete_icon"></i></a>
													<?php } ?>	
														</td>
													</tr>
