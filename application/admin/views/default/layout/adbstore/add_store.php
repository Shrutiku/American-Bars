...
<div class="page_content">
			<div class="container_fluid">
				...
				echo form_open_multipart('adbstore/add_store',$attributes);
			  ?>
			  						...
			  						<div class="control_group">
										...
										<div class="controls">
											<input maxlength="40" type="text" placeholder="Product Name...." class="m_wrap wid360" name="product_name" id="product_name" value="<?php echo $product_name; ?>">
										</div>										
										<div class="clear"></div>
									</div>
									<div class="control_group">
								...
								<?php /*	<div class="control-group">
									...
									<div class="control-group">
											...
											<div class="span3">
												<?php if($i==0){ ?>
												<a href="javascript:void(0);" id="add_row" name="add_row" class="table_icon btn blue margin-left-10 margin-top-10"><i class="comon_icon addmore_icon"></i></a>
												<?php }else{ ?>
												<a href="javascript://" style="margin-left: 10px;" class="table_icon btn red margin-top-10" onclick="removeImageDiveAjax('<?php echo $im->product_image_id ?>')"><i class="comon_icon delete_icon"></i></a>
												<?php } ?>
									...
									<div class="control_group">
										...
										<div class="controls" >
											...
											<!-- <?php  $c=($color!='')?explode(',',$color):array();
												
												?>
											<select data-placeholder="Select Color" class="m_wrap wid360 " multiple="multiple" id="color1" tabindex="6" name="color[]">
                                                <option value="">&nbsp;</option>
                                                <?php
                                               
                                                if($colorlist!=''){
                                                 foreach($colorlist as $cl){?>
                                                    <option value="<?php echo $cl->Color_id ?>" <?php  echo (in_array($cl->Color_id,$c)?'selected':'') ?>><?php echo ucwords($cl->color_name); ?></option>
                                            <?php   }} ?>
                                            </select>-->
									...
									<div class="control_group">
										...
										<div class="controls" >
											...
											<!-- <?php 
											 $si123=($size!='')?explode(',',$size):array();
                                                
												?>
											<select data-placeholder="Select Size" class="m_wrap wid360 " multiple="multiple" id="size" tabindex="6" name="size[]">
                                                <option value="">&nbsp;</option>
                                                <?php
                                               
                                                if($sizelist!=''){
                                                 foreach($sizelist as $s){?>
                                                    <option value="<?php echo $s->Size_id ?>" <?php  echo (in_array($s->Size_id,$si123)?'selected':'') ?>><?php echo ucwords($s->size_name); ?></option>
                                            <?php   }} ?>
                                            </select> -->
									...
									<div class="control_group">
										...
										<div class="controls">
											<input type="text" placeholder="Meta Title" class="m_wrap wid360" name="store_meta_title" id="store_meta_title" value="<?php echo $store_meta_title; ?>">
										</div>										
										<div class="clear"></div>
									</div>
									...
									<div class="control_group">
										...
										<div class="controls">
											<input type="text" placeholder="Meta Keyword" class="m_wrap wid360" name="store_meta_keyword" id="store_meta_keyword" value="<?php echo $store_meta_keyword; ?>">
										</div>										
										<div class="clear"></div>
									</div>
									...
									<div class="control_group">
										...
										<div class="controls">
											<textarea id="store_meta_description" cols="10" rows="4" name="store_meta_description" class="wid360 m_wrap"><?php echo $store_meta_description; ?></textarea>
										</div>										
										<div class="clear"></div>
									</div>
						...
						{?>
						<input type="button" name="Cancel" value="Cancel" class="btn red  fl_left" onClick="location.href='<?php echo base_url(); ?>adbstore/<?php echo $redirect_page.'/'.$limit.'/'.$offset?>'" />
						<?php }else
						{?>
						<input type="button" name="Cancel" value="Cancel" class="btn red  fl_left" onClick="location.href='<?php echo base_url(); ?>adbstore/<?php echo $redirect_page.'/'.$limit.'/'.$option.'/'.$keyword.'/'.$offset?>'" />
...
<script>
	$('#add_row').click(function(){
	  //if(parseInt($('#cntpro').val())<7){
		...
		$('#cntpro').val(cnt)
		$('#inner').append('<div class="row-fluid  margin-top-10" id="img_'+cnt+'" style="display:none;"><div class=""><div class="fileupload fileupload-new pull-left" data-provides="fileupload"><div class="uneditable-input"><i class="icon-file fileupload-exists"></i><span class="fileupload-preview"></span></div><span class="btn btn-file"><span class="fileupload-new">Select file</span><span class="fileupload-exists">Change</span><input type="file" class="default"  accept="image/*" name="photo_image[]" id="photo_image_'+cnt+'" /></span><a href="javascript://" class="btn fileupload-exists" data-dismiss="fileupload">Remove</a></div><a href="javascript://" class="table_icon btn red margin-left-10 margin-top-10" onclick="removeImageDive(\''+cnt+'\')"><i class="comon_icon delete_icon"></i></a></div></div></div><div class="clear"></div>');
		$('#img_'+cnt).slideDown();
		//}else{
		//$('#cantADD').show();	
		//}
