...
$('.sorted_table').sortable({
  ...
  onDrop: function (item, container, _super) {
         ...
         console.log(ids);
           ...
           $.ajax({
                url: "<?php echo site_url('album/reorder');?>",
                type: "post",
                data: {id:ids},
                cache: false,
                dataType: "json",
...
$(document).ready(function(){
	...
	function delete_rec(id,redirectpage,option,keyword,$gallery_id,limit,offset)
	{
		...
		if(ans)
		{
			location.href = "<?php echo base_url(); ?>album/delete_gallery/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+$gallery_id+"/"+limit+"/"+offset;
		}else{
			return false;
		}
	}
	...
	function getlimit(limit)
	{
		...
		{
		 window.location.href= '<?php echo base_url();?>album/list_album/'+limit+'/<?php echo $bar_id ?>';
		}
	...
	function getsearchlimit(limit)
	{
		...
		else
		{
			...
			window.location.href='<?php echo base_url();?>album/search_list_album/'+limit+'/<?php echo $option.'/'.$keyword; ?>';
		}
...
<div class="page_content">
    ...
    <?php } ?>
				<div class="row_fluid"> 
					...
					echo form_open('album/search_list_album/'.$limit,$attributes);?>
					<div class="fl_left">
					...
					<span class="sspan fl_left ">&nbsp;&nbsp;&nbsp;Search By</span>&nbsp;
                            ...
                            <input type="submit" name="submit" id="submit" value="Search" class="btn blue  fl_left mar10" /> 
                             ...
                             <input type="button" name="refresh" id="refresh" value="Refresh" class="btn blue  fl_left mar10" onclick="document.location.href = '<?php echo site_url("album/list_album/".$limit.'/'.$bar_id); ?>'" /> 
								</form>
								</div>
									<div class="fl_right">
					...
					<?php echo anchor('album/add_album/'.$bar_id,'Add New', 'class="btn blue  fl_left mar_r_5" id="addgallery"'); ?>
					...
					<div class="clear"></div>		
									...
									<div class="clear"></div>
			<form name="frm_listlogin" id="frm_listlogin" action="<?php echo base_url();?>album/action_gallery" method="post">
				   ...
				   <input type="hidden" name="redirect_page" id="redirect_page" value="<?php echo $redirect_page;?>"/>
								...
								if($result)
								{
									...
									<td style="text-align: center;">	
										...
										<?php echo  anchor('album/edit_album/'.$row->bar_gallery_id.'/'.$redirect_page.'/'.$option.'/'.$keyword.'/'.$limit.'/'.$offset."/".$bar_id,'<i class="comon_icon edit_icon"></i>','class="table_icon btn blue" id="gallery_'.$row->bar_gallery_id.'" title="Edit gallery"');  ?>
