...
<script type="text/javascript" language="javascript">
	...
	function getlimit(limit)
	{
		...
		{
			window.location.href='<?php echo base_url();?>membership/list_membership/'+limit+'/';
		}
	...
	function getsearchlimit(limit)
	{
		...
		else
		{
			...
			window.location.href='<?php echo base_url();?>membership/search_list_membership/'+limit+'/<?php echo $option.'/'.$keyword; ?>';
		}
	...
	function gomain(x)
	{
		...
		if(x == 'all')
		{
			window.location.href= '<?php echo base_url();?>membership/list_membership';
		}
...
<div class="page_content">
			...
			<div class="container_fluid">
				...
				<div class="row_fluid"> 
					<h3 class="page_title">Membership Listing</h3>
    ...
    <?php } ?>
				<div class="row_fluid"> 
					...
					echo form_open('membership/search_list_membership/'.$limit,$attributes);?>
					<div class="fl_left">
					...
					<?php //echo anchor('membership/add_membership','Add New', 'class="btn blue  fl_left mar_r_5" id="addmembership"'); ?>
					...
					<div class="clear"></div>		
									...
									<div class="clear"></div>
			<form name="frm_listlogin" id="frm_listlogin" action="<?php echo base_url();?>membership/action_membership" method="post">
	<input type="hidden" name="offset" id="offset" value="<?php echo $offset; ?>" />
				   ...
				   <div class="scroll-pane horizontal-only" >
									<table class="table border" >
												...
												<th class="sorting_disabled" style="width: 6%;">Plan Title</th>
												<th class="sorting" style="width: 12%;">Category</th>
								...
								if($result)
								{
									...
									<td  style="width: 30px;"><input type="checkbox" name="chk[]" value="<?php echo $row->membership_plan_id ?>" class="chk"  /></td>
														<td class=" sorting_1"><?php echo $row->plan_title; ?></td>
														...
														<?php 
																...
																echo anchor('membership/edit_membership/'.$row->membership_plan_id.'/'.$redirect_page.'/'.$option.'/'.$keyword.'/'.$limit.'/'.$offset,'<i class="comon_icon edit_icon"></i>','class="table_icon btn blue" id="membership_'.$row->membership_plan_id.'" title="Edit Admin"'); 
														?>
														
														<a class="table_icon btn red" href="javascript://" onClick="delete_rec('<?php echo $row->membership_plan_id; ?>','<?php echo $redirect_page;?>','<?php echo $option?>','<?php echo $keyword?>','<?php echo $limit?>','<?php echo $offset; ?>')" title="Delete"><i class="comon_icon delete_icon"></i></a>
														</td>
													</tr>
								<?php $i++;} }else{ ?>
