...
$(document).ready(function(){
	...
	{
		var ans = confirm("Are you sure, you want to delete job?");
		if(ans)
		{
			location.href = "<?php echo base_url(); ?>job/delete_job/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+limit+"/"+offset;
		}else{
			return false;
		}
	}
	...
	function getlimit(limit)
	{
		...
		{
			window.location.href='<?php echo base_url();?>job/list_job/'+limit+'/';
		}
	...
	function getsearchlimit(limit)
	{
		...
		else
		{
			...
			window.location.href='<?php echo base_url();?>job/search_list_job/'+limit+'/<?php echo $option.'/'.$keyword; ?>';
		}
	...
	function gomain(x)
	{
		...
		if(x == 'all')
		{
			window.location.href= '<?php echo base_url();?>job/list_job/';
		}
				...
				<div class="row_fluid"> 
					<div class="portlet blue">
						<div class="portlet-title">
							<div class="caption fl_left"> Jobs </div>
							<div class="fl_right">
					...
					echo form_open('job/search_list_job/'.$limit,$attributes);?>
					<div class="fl_left">
					...
					<span class="sspan fl_left ">&nbsp;&nbsp;&nbsp;Search By</span>&nbsp;
                            <select name="option" id="option" onchange="gomain(this.value)"  class="m_wrap fl_left mar0" style="padding:6px;">
                                ...
                                <option value="job_title" <?php if($option=='job_title'){?> selected="selected"<?php }?>>Jobs Title</option>
								<option value="job_category" <?php if($option=='job_category'){?> selected="selected"<?php }?>>Jobs Category</option>  
                            </select>
					</div>
                            ...
                            <input type="submit" name="submit" id="submit" value="Search" class="btn blue  fl_left mar10" /> 
                             ...
                             <input type="button" name="refresh" id="refresh" value="Refresh" class="btn blue  fl_left mar10" onclick="document.location.href = '<?php echo site_url("job"); ?>'" /> 
								</form>
								</div>
									<div class="fl_right">
					...
					<?php echo anchor('job/add_job','Add New', 'class="btn blue  fl_left mar_r_5" id="addjob"'); ?>
					...
					<div class="clear"></div>		
									...
									<div class="clear"></div>
					<form name="frm_listlogin" id="frm_listlogin" action="<?php echo base_url();?>job/action_job" method="post">
				   ...
				   <input type="hidden" name="redirect_page" id="redirect_page" value="<?php echo $redirect_page;?>"/>
								<div class="scroll-pane horizontal-only" >	<table class="table border" >
												...
												<th class="sorting_disabled" style="width: 6%;">Jobs Title</th>
												<th class="sorting" style="width: 5%;">Description</th>
												<th class="sorting" style="width: 5%;">Jobs Category</th>
												<th class="sorting" style="width: 5%;">Added By</th>
								...
								if($result)
								{
									...
									<td  style="width: 30px;"><input type="checkbox" name="chk[]" value="<?php echo $row->job_id ?>" class="chk"  /></td>
														<td class=" sorting_1"><?php echo $row->job_title; ?></td>
														<td><?php echo $row->job_desc; ?></td>
														<td><?php echo ucfirst(str_replace("_"," ",$row->job_category)); ?></td>
														...
														<?php 
							//FUTURE 
							// 	echo anchor('job/edit_job/'.$row->job_id.'/'.$redirect_page.'/'.$option.'/'.$keyword.'/'.$limit.'/'.$offset,'<i class="comon_icon edit_icon"></i>','class="table_icon btn blue" id="job_'.$row->job_id.'" title="Edit job"'); 
						?>
														<!--Futture enhancement															
															<a class="btn green table_icon" href="javascript:;"><i class="comon_icon view_icon"></i></a>-->
														<?php echo  anchor('job/edit_job/'.$row->job_id.'/'.$redirect_page.'/'.$option.'/'.$keyword.'/'.$limit.'/'.$offset,'<i class="comon_icon edit_icon"></i>','class="table_icon btn blue" id="job_'.$row->job_id.'" title="Edit job"');  ?>
														<a class="table_icon btn red" href="javascript://" onClick="delete_rec('<?php echo $row->job_id; ?>','<?php echo $redirect_page;?>','<?php echo $option?>','<?php echo $keyword?>','<?php echo $limit?>','<?php echo $offset; ?>')" title="Delete"><i class="comon_icon delete_icon"></i></a>
														</td>
													</tr>
								<?php $i++;} }else{ ?>
