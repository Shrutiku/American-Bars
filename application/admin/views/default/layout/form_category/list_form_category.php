...
<script type="text/javascript" language="javascript">
	...
	{
		var ans = confirm("Are you sure, you want to delete Form_category?");
		if(ans)
		{
			location.href = "<?php echo base_url(); ?>form_category/delete_form_category/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+limit+"/"+offset;
		}else{
			return false;
		}
	}
	...
	function getlimit(limit)
	{
		...
		{
			window.location.href='<?php echo base_url();?>form_category/list_form_category/'+limit+'/';
		}
	...
	function getsearchlimit(limit)
	{
		...
		else
		{
			...
			window.location.href='<?php echo base_url();?>form_category/search_list_form_category/'+limit+'/<?php echo $option.'/'.$keyword; ?>';
		}
	...
	function gomain(x)
	{
		...
		if(x == 'all')
		{
			window.location.href= '<?php echo base_url();?>form_category/list_form_category';
		}
...
<div class="page_content">
			...
			<div class="container_fluid">
				...
				<div class="row_fluid"> 
					<h3 class="page_title">Forum Category Listing</h3>
    ...
    <?php } ?>
				<div class="row_fluid"> 
					...
					echo form_open('form_category/search_list_form_category/'.$limit,$attributes);?>
					<div class="fl_left">
					...
					<span class="sspan fl_left ">&nbsp;&nbsp;&nbsp;Search By</span>&nbsp;
                            <select name="option" id="option" onchange="gomain(this.value)"  class="m_wrap fl_left mar0" style="padding:6px;">
                                ...
                                <option value="forum_category_name" <?php if($option=='forum_category_name'){?> selected="selected"<?php }?>>Category name</option>  
							...
							<input type="button" name="refresh" id="submit" value="Refresh" class="btn blue  fl_left mar10" onclick="document.location.href ='<?php echo site_url("form_category/list_form_category"); ?>'" /> 
								</form>
								</div>
									<div class="fl_right">
					...
					<?php echo anchor('form_category/add_form_category','Add New', 'class="btn blue  fl_left mar_r_5" id="addform_category"'); ?>
					...
					<div class="clear"></div>		
									...
									<div class="clear"></div>
			<form name="frm_listlogin" id="frm_listlogin" action="<?php echo base_url();?>form_category/action_form_category" method="post">
	<input type="hidden" name="offset" id="offset" value="<?php echo $offset; ?>" />
				   ...
				   <div class="scroll-pane horizontal-only" >
									<table class="table border" >
												...
												<th class="sorting_disabled" style="width: 6%;">Forum Category Name</th>
								...
								if($result)
								{
									...
									<td  style="width: 30px;"><input type="checkbox" name="chk[]" value="<?php echo $row->forum_category_id ?>" class="chk"  /></td>
														...
														<?php 
																...
																echo anchor('form_category/edit_form_category/'.$row->forum_category_id.'/'.$redirect_page.'/'.$option.'/'.$keyword.'/'.$limit.'/'.$offset,'<i class="comon_icon edit_icon"></i>','class="table_icon btn blue" id="form_category_'.$row->forum_category_id.'" title="Edit Admin"'); 
														?>
														
														<a class="table_icon btn red" href="javascript://" onClick="delete_rec('<?php echo $row->forum_category_id; ?>','<?php echo $redirect_page;?>','<?php echo $option?>','<?php echo $keyword?>','<?php echo $limit?>','<?php echo $offset; ?>')" title="Delete"><i class="comon_icon delete_icon"></i></a>
														</td>
													</tr>
								<?php $i++;} }else{ ?>
