...
<script type="text/javascript" language="javascript">
	...
	{
		var ans = confirm("Are you sure, you want to delete color?");
		if(ans)
		{
			location.href = "<?php echo base_url(); ?>color/delete_color/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+limit+"/"+offset;
		}else{
			return false;
		}
	}
	...
	function getlimit(limit)
	{
		...
		{
			window.location.href='<?php echo base_url();?>color/list_color/'+limit+'/';
		}
	...
	function getsearchlimit(limit)
	{
		...
		else
		{
			...
			window.location.href='<?php echo base_url();?>color/search_list_color/'+limit+'/<?php echo $option.'/'.$keyword; ?>';
		}
	...
	function gomain(x)
	{
		...
		if(x == 'all')
		{
			window.location.href= '<?php echo base_url();?>color/list_color';
		}
...
<div class="page_content">
			...
			<div class="container_fluid">
				...
				<div class="row_fluid"> 
					<h3 class="page_title">Color Listing</h3>
				...
				<div class="row_fluid"> 
					...
					<span class="sspan fl_left ">&nbsp;&nbsp;&nbsp;Search By</span>&nbsp;
                            <select name="option" id="option" onchange="gomain(this.value)"  class="m_wrap fl_left mar0" style="padding:6px;">
                                ...
                                <option value="color_name" <?php if($option=='color_name'){?> selected="selected"<?php }?>>Color name</option>  
							...
							<input type="button" name="refresh" id="submit" value="Refresh" class="btn blue  fl_left mar10" onclick="document.location.href ='<?php echo site_url("color/list_color"); ?>'" /> 
								</form>
								</div>
									<div class="fl_right">
					...
					<?php echo anchor('color/add_color','Add New', 'class="btn blue  fl_left mar_r_5" id="addcolor"'); ?>
					...
					<div class="clear"></div>		
									...
									<div class="clear"></div>
			<form name="frm_listlogin" id="frm_listlogin" action="<?php echo base_url();?>color/action_color" method="post">
	<input type="hidden" name="offset" id="offset" value="<?php echo $offset; ?>" />
				   ...
				   <div class="scroll-pane horizontal-only" >
									<table class="table border" >
												...
												<th class="sorting_disabled" style="width: 6%;">Color Name</th>
								...
								if($result)
								{
									...
									<td  style="width: 30px;"><input type="checkbox" name="chk[]" value="<?php echo $row->Color_id ?>" class="chk"  /></td>
														<td class=" sorting_1"><?php echo $row->color_name; ?></td>
														<td class="center">
															<?php $cls = ($row->status=='active')?'purple':'yellow';?>
															<span class="<?php echo $cls;?>"><?php echo ucfirst($row->status); ?></span>
														</td>
														
														<!-- <td class="center"><?php if($this->session->userdata('color_type')=='1'){echo anchor('rights/assign_rights/'.$row->Color_id.'/'.$limit.'/'.$offset,'<i class="halflings-icon ok"></i>'); }else{ echo "N/A";} ?></td>
														<td>
														<a class="table_icon btn green" href="<?php echo site_url('color/list_logactivity/'.$row->Color_id);?>">
														...
														<?php 
																...
																echo anchor('color/edit_color/'.$row->Color_id.'/'.$redirect_page.'/'.$option.'/'.$keyword.'/'.$limit.'/'.$offset,'<i class="comon_icon edit_icon"></i>','class="table_icon btn blue" id="color_'.$row->Color_id.'" title="Edit Admin"'); 
														?>
														
														<a class="table_icon btn red" href="javascript://" onClick="delete_rec('<?php echo $row->Color_id; ?>','<?php echo $redirect_page;?>','<?php echo $option?>','<?php echo $keyword?>','<?php echo $limit?>','<?php echo $offset; ?>')" title="Delete"><i class="comon_icon delete_icon"></i></a>
														</td>
													</tr>
								<?php $i++;} }else{ ?>
