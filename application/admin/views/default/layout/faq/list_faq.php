<script type="text/javascript" language="javascript">
	function delete_rec(id,redirectpage,option,keyword,limit,offset) {
		var ans = confirm("Are you sure, you want to delete faq?");
		if(ans) {
			location.href = "<?php echo base_url(); ?>faq/delete_faq/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+limit+"/"+offset;
		}else{
			return false;
		}
	}
	...
	function getlimit(limit) {
		...
		if(limit=='0'){
			...
			window.location.href='<?php echo base_url();?>faq/list_faq/'+limit+'/';
		}
	}	
	...
	function getsearchlimit(limit) {
		if(limit=='0') {
			...
			window.location.href='<?php echo base_url();?>faq/search_list_faq/'+limit+'/<?php echo $option.'/'.$keyword; ?>';
		}
	...
	function gomain(x) {
		if(x == 'all')
		{
			window.location.href= '<?php echo base_url();?>faq/list_faq';
		}
	}
...
<div class="page_content">
			...
			<div class="container_fluid">
				...
				<div class="row_fluid"> 
					<h3 class="page_title">List FAQ</h3>
					...
					<?php 
						...
						if($msg != ""){
							...
							if($msg == "faq_limit"){ $error = FAQ_LIMIT;}
				...
				<div class="row_fluid"> 
					<div class="portlet blue">
						<div class="portlet-title">
							<div class="caption fl_left">List FAQ</div>
								<div class="fl_right">
						...
						<div class="portlet-body form">
									...
									<div class="sdrop fl_left wid228">
										<span class="sspan fl_left ">&nbsp;&nbsp;&nbsp;Search By</span>&nbsp;
											<select name="option" id="option" onchange="gomain(this.value)"  class="m_wrap fl_left mar0" style="padding:6px;">
												...
												<option value="faq_name" <?php if($option=='faq_question'){?> selected="selected"<?php }?>>FAQ Question</option>  
							...
							<div class="fl_right">
									...
									<?php echo anchor('faq/add_faq','Add New', 'class="btn blue  fl_left mar_r_5" id="addfaq"'); ?>
									...
									<div class="clear"></div>
								<form name="frm_listlogin" id="frm_listlogin" action="<?php echo base_url();?>faq/action_faq" method="post">
									...
									<table class="table border" >
										<thead>
											<tr>
											<th class="sorting_disabled" style="width: 1%;"> <input type="checkbox" id="titleCheck" name="titleCheck"  onclick="setchecked('chk[]')" /></th>
											<th class="sorting" style="width: 80%;">FAQ Question</th>
											<th class="sorting" style="width: 10%;">Status</th>
											<th class="sorting" style="width: 9%;">Action</th>
											</tr>
										</thead>
										<tbody role="alert" aria-live="polite" aria-relevant="all">	
											...
											if($result)
											{
												...
												<tr class="<?php echo $cls ?>">
													<td  style="width: 30px;"><input type="checkbox" name="chk[]" value="<?php echo $row->faq_id ?>" class="chk"  /></td>
													<td class=" sorting_1"><?php echo $row->faq_question;?></td>
													<td class="center">
														<?php $cls = ($row->status=='Active')?'purple':'yellow';?>
														<span class="<?php echo $cls;?>"><?php echo ucfirst($row->status); ?></span>
													</td>
													<td>
														<?php echo anchor('faq/edit_faq/'.$row->faq_id.'/'.$redirect_page.'/'.$option.'/'.$keyword.'/'.$limit.'/'.$offset,'<i class="comon_icon edit_icon"></i>','class="table_icon btn blue" id="faq_'.$row->faq_id.'" title="Edit faq"'); ?>
														<a class="table_icon btn red" href="javascript://" onClick="delete_rec('<?php echo $row->faq_id; ?>','<?php echo $redirect_page;?>','<?php echo $option?>','<?php echo $keyword?>','<?php echo $limit?>','<?php echo $offset; ?>')" title="Delete"><i class="comon_icon delete_icon"></i></a>
													</td>
												</tr>
											<?php $i++;} }else{ ?>															
												<tr class="odd">
