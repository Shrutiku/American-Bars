<script type="text/javascript" language="javascript">
	...
	{
		var ans = confirm("Are you sure, you want to delete dictionary?");
		if(ans)
		{
			location.href = "<?php echo base_url(); ?>dictionary/delete_dictionary/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+limit+"/"+offset;
		}else{
			return false;
		}
	}
	...
	function getlimit(limit)
	{
		...
		{
			window.location.href='<?php echo base_url();?>dictionary/list_dictionary/'+limit+'/';
		}
	...
	function getsearchlimit(limit)
	{
		...
		else
		{
			...
			window.location.href='<?php echo base_url();?>dictionary/search_list_dictionary/'+limit+'/<?php echo $option.'/'.$keyword; ?>';
		}
	...
	function gomain(x)
	{
		...
		if(x == 'all')
		{
			window.location.href= '<?php echo base_url();?>dictionary/list_dictionary/';
		}
...
function setaction(elename, actionval, actionmsg, formname) {
	...
	function reply_message(id,redirectpage,option,keyword,limit,offset){
		location.href = "<?php echo base_url(); ?>dictionary/view/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+limit+"/"+offset;
	}
...
<div class="page_content">
			...
			<div class="container_fluid">
				...
				<div class="row_fluid"> 
					<h3 class="page_title">Dictionary Listing</h3>
    ...
    <?php } ?>
				<div class="row_fluid"> 
					...
					echo form_open('dictionary/search_list_dictionary/'.$user_id.'/'.$limit,$attributes);?>
					<div class="fl_left">
					...
					<span class="sspan fl_left ">&nbsp;&nbsp;&nbsp;Search By</span>&nbsp;
                            ...
                            <input type="submit" name="submit" id="submit" value="Search" class="btn blue  fl_left mar0" /> 
									...
									<div class="fl_right">	
				<?php echo anchor('dictionary/import','Import CSV', 'class="btn blue  fl_left mar_r_5" id="addbeer"'); ?>
					<a href="<?php echo front_base_url().'/upload/dictionary.csv';?>" class='btn purple fl_left mar_r_5'>Demo Csv</a>																																	
					<?php echo anchor(site_url("dictionary/add_dictionary/"),'Add New', 'class="btn blue  fl_left mar_r_5" id="addadmin"'); ?>
					...
					<div class="clear"></div>																					
									...
									<div class="clear"></div>
			<form name="frm_listlogin" id="frm_listlogin" action="<?php echo base_url();?>dictionary/action_dictionary" method="post">
				   ...
				   <div class="scroll-pane horizontal-only" >
									...
									<table class="table border" >
												...
												<th class="sorting_disabled">Dictionary Title</th>
												<th class=""> View </th>
								...
								if($result)
								{
									...
									<td  style="width: 30px;"><input type="checkbox" name="chk[]" value="<?php echo $row->dictionary_id ?>" class="chk"  /></td>
										<td class=" sorting_1"><?php echo $row->dictionary_title; ?></td>
										<td align="center" style="text-align:center;">
														<a class="btn blue" href="javascript://" onclick="reply_message('<?php echo $row->dictionary_id; ?>','<?php echo $redirect_page;?>','<?php echo $option?>','<?php echo $keyword?>','<?php echo $limit?>','<?php echo $offset; ?>');" title="View">View</a>
														</td>
										<td class="center">
										...
										<td>
										<?php 
												echo anchor('dictionary/edit_dictionary/'.$row->dictionary_id.'/'.$redirect_page.'/'.$option.'/'.$keyword.'/'.$limit.'/'.$offset,'<i class="comon_icon edit_icon"></i>','class="table_icon btn blue" id="dictionary_'.$row->dictionary_id.'" title="Edit Admin"'); 
										?>
										<a class="table_icon btn red" href="javascript://" onClick="delete_rec('<?php echo $row->dictionary_id;?>','<?php echo $redirect_page;?>','<?php echo $option?>','<?php echo $keyword?>','<?php echo $limit?>','<?php echo $offset; ?>')" title="Delete"><i class="comon_icon delete_icon"></i></a>
										</td>
