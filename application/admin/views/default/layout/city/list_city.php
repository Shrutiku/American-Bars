<script type="text/javascript" language="javascript">
	...
	function delete_rec(id,redirectpage,option,keyword,limit,offset)
	{
		...
		if(ans)
		{
			location.href = "<?php echo base_url(); ?>city/delete_city/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+limit+"/"+offset;
		}else{
			return false;
		}
	}
...
<div class="page_content">
			...
			<div class="container_fluid">
				...
				<div class="row_fluid"> 
					<h3 class="page_title">City listing</h3>
				...
				<div class="row_fluid"> 
					<div class="portlet blue">
						<div class="portlet-title">
							<div class="caption fl_left">City listing</div>
							<div class="fl_right">
					...
					<span class="sspan fl_left ">&nbsp;&nbsp;&nbsp;Search By</span>&nbsp;
                            <select name="option" id="option" onchange="gomain(this.value)"  class="m_wrap fl_left mar0" style="padding:6px;">
                                ...
                                <option value="city" <?php if($option=='city'){?> selected="selected"<?php }?>>City Name</option>  
                                <option value="state" <?php if($option=='state'){?> selected="selected"<?php }?>>State Name</option>
                                <option value="country" <?php if($option=='country'){?> selected="selected"<?php }?>>Country Name</option>
					...
					<?php echo anchor('city/add_city','Add New', 'class="btn blue  fl_left mar_r_5" id="addcity"'); ?>
					...
					<div class="clear"></div>		
									...
									<div class="clear"></div>
			<form name="frm_listlogin" id="frm_listlogin" action="<?php echo base_url();?>city/action_city" method="post">
	<input type="hidden" name="offset" id="offset" value="<?php echo $offset; ?>" />
				   ...
				   <input type="hidden" name="redirect_page" id="redirect_page" value="<?php echo $redirect_page;?>"/>
							<div class="scroll-pane horizontal-only" >		<table class="table border" >
												...
												<th class="sorting_disabled" style="width: 6%;">City Name</th>
												<th class="sorting_disabled" style="width: 6%;">State Name</th>
												<th class="sorting" style="width: 12%;">Country Name</th>
												<th class="sorting" style="width: 12%;">Pincode</th>
												<th class="sorting" style="width: 12%;">Status</th>
								...
								if($result)
								{
									...
									<td  style="width: 30px;"><input type="checkbox" name="chk[]" value="<?php echo $row->city_id ?>" class="chk"  /></td>
														<td class=" sorting_1"><?php echo $row->city_name; ?></td>
								...
								echo anchor('city/edit_city/'.$row->city_id.'/'.$redirect_page.'/'.$option.'/'.$keyword.'/'.$limit.'/'.$offset,'<i class="comon_icon edit_icon"></i>','class="table_icon btn blue" id="city_'.$row->city_id.'" title="Edit city"'); 
						?>
														<!--Future enhancement
															<a class="btn green table_icon" href="javascript:;"><i class="comon_icon view_icon"></i></a>
														<a class="table_icon btn blue" href="javascript:;"><i class="comon_icon edit_icon"></i></a>-->
														<a class="table_icon btn red" href="javascript://" onClick="delete_rec('<?php echo $row->city_id; ?>','<?php echo $redirect_page;?>','<?php echo $option?>','<?php echo $keyword?>','<?php echo $limit?>','<?php echo $offset; ?>')" title="Delete"><i class="comon_icon delete_icon"></i></a>
														</td>
													</tr>
								<?php $i++;} }else{ ?>
