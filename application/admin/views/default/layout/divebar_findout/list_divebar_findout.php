<script type="text/javascript" language="javascript">
	...
	{
		var ans = confirm("Are you sure, you want to delete divebar_findout?");
		if(ans)
		{
			location.href = "<?php echo base_url(); ?>divebar_findout/delete_divebar_findout/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+limit+"/"+offset;
		}else{
			return false;
		}
	}
	...
	function getlimit(limit)
	{
		...
		{
			window.location.href='<?php echo base_url();?>divebar_findout/list_divebar_findout/'+limit+'/';
		}
	...
	function getsearchlimit(limit)
	{
		...
		else
		{
			...
			window.location.href='<?php echo base_url();?>divebar_findout/search_list_divebar_findout/'+limit+'/<?php echo $option.'/'.$keyword; ?>';
		}
	...
	function gomain(x)
	{
		...
		if(x == 'all')
		{
			window.location.href= '<?php echo base_url();?>divebar_findout/list_divebar_findout/';
		}
...
function setaction(elename, actionval, actionmsg, formname) {
	...
	function reply_message(id,redirectpage,option,keyword,limit,offset){
		location.href = "<?php echo base_url(); ?>divebar_findout/view/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+limit+"/"+offset;
	}
...
<div class="page_content">
			...
			<div class="container_fluid">
				...
				<div class="row_fluid"> 
					<h3 class="page_title">Divebar Findout Listing</h3>
    ...
    <?php } ?>
				<div class="row_fluid"> 
					...
					echo form_open('divebar_findout/search_list_divebar_findout/'.$user_id.'/'.$limit,$attributes);?>
					<div class="fl_left">
					...
					<span class="sspan fl_left ">&nbsp;&nbsp;&nbsp;Search By</span>&nbsp;
                            ...
                            <input type="submit" name="submit" id="submit" value="Search" class="btn blue  fl_left mar0" /> 
									...
									<div class="fl_right">	
					<?php echo anchor(site_url("divebar_findout/add_divebar_findout/"),'Add New', 'class="btn blue  fl_left mar_r_5" id="addadmin"'); ?>
					...
					<div class="clear"></div>																					
									...
									<div class="clear"></div>
			<form name="frm_listlogin" id="frm_listlogin" action="<?php echo base_url();?>divebar_findout/action_divebar_findout" method="post">
				   ...
				   <div class="scroll-pane horizontal-only" >
									...
									<table class="table border" >
												...
												<th class="sorting_disabled">Divebar Findout Title</th>
								...
								if($result)
								{
									...
									<td  style="width: 30px;"><input type="checkbox" name="chk[]" value="<?php echo $row->divebar_findout_id ?>" class="chk"  /></td>
										<td class=" sorting_1"><?php echo $row->divebar_findout_title; ?></td>
										...
										<td>
										<?php 
												echo anchor('divebar_findout/edit_divebar_findout/'.$row->divebar_findout_id.'/'.$redirect_page.'/'.$option.'/'.$keyword.'/'.$limit.'/'.$offset,'<i class="comon_icon edit_icon"></i>','class="table_icon btn blue" id="divebar_findout_'.$row->divebar_findout_id.'" title="Edit Admin"'); 
										?>
										<a class="table_icon btn red" href="javascript://" onClick="delete_rec('<?php echo $row->divebar_findout_id;?>','<?php echo $redirect_page;?>','<?php echo $option?>','<?php echo $keyword?>','<?php echo $limit?>','<?php echo $offset; ?>')" title="Delete"><i class="comon_icon delete_icon"></i></a>
										</td>
