...
function setaction(elename, actionval, actionmsg, formname) {
	...
	function reply_message(id,redirectpage,option,keyword,limit,offset){
		location.href = "<?php echo base_url(); ?>article/view/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+limit+"/"+offset;
	}
...
<div class="page_content">
    ...
    <?php } ?>
				<div class="row_fluid"> 
					...
					echo form_open('article/search_list_article/'.$user_id.'/'.$limit,$attributes);?>
					<div class="fl_left">
					...
					<span class="sspan fl_left ">&nbsp;&nbsp;&nbsp;Search By</span>&nbsp;
                            ...
                            <input type="submit" name="submit" id="submit" value="Search" class="btn blue  fl_left mar0" /> 
									...
									<div class="fl_right">																												
					<?php echo anchor(site_url("article/add_article/"),'Add New', 'class="btn blue  fl_left mar_r_5" id="addadmin"'); ?>
			...
			<form name="frm_listlogin" id="frm_listlogin" action="<?php echo base_url();?>article/action_article" method="post">
				   ...
				   <div class="scroll-pane horizontal-only" >
									...
									<table class="table border" >
												...
												<th class="sorting_disabled">Article</th>
												<th class=""> View </th>
								...
								if($result)
								{
									...
									<td  style="width: 30px;"><input type="checkbox" name="chk[]" value="<?php echo $row->blog_id ?>" class="chk"  /></td>
										<td class=" sorting_1"><?php echo $row->blog_title; ?></td>
										<td align="center" style="text-align:center;">
														<a class="table_icon btn blue" href="javascript://" onclick="reply_message('<?php echo $row->blog_id; ?>','<?php echo $redirect_page;?>','<?php echo $option?>','<?php echo $keyword?>','<?php echo $limit?>','<?php echo $offset; ?>');" title="View"><i class="comon_icon view_icon"></i></a>
														</td>
										<td class="center">
										...
										<td>
										<?php 
												echo anchor('article/edit_article/'.$row->blog_id.'/'.$redirect_page.'/'.$option.'/'.$keyword.'/'.$limit.'/'.$offset,'<i class="comon_icon edit_icon"></i>','class="table_icon btn blue" id="blog_'.$row->blog_id.'" title="Edit Admin"'); 
										?>
										<a class="table_icon btn red" href="javascript://" onClick="delete_rec('<?php echo $row->blog_id;?>','<?php echo $redirect_page;?>','<?php echo $option?>','<?php echo $keyword?>','<?php echo $limit?>','<?php echo $offset; ?>')" title="Delete"><i class="comon_icon delete_icon"></i></a>
										</td>
