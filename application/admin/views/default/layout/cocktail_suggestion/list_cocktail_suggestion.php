...
$(document).ready(function(){
	...
	{
		var ans = confirm("Are you sure, you want to delete cocktail_suggestion?");
		if(ans)
		{
			location.href = "<?php echo base_url(); ?>cocktail_suggestion/delete_cocktail_suggestion/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+limit+"/"+offset;
		}else{
			return false;
		}
	}
	...
	function getlimit(limit)
	{
		...
		{
			window.location.href='<?php echo base_url();?>cocktail_suggestion/list_cocktail_suggestion/'+limit+'/<?php echo $bars_id ?>';
		}
	...
	function getsearchlimit(limit)
	{
		...
		else
		{
			...
			window.location.href='<?php echo base_url();?>cocktail_suggestion/search_list_cocktail_suggestion/'+limit+'/<?php echo $bars_id.'/'.$option.'/'.$keyword; ?>';
		}
	...
	function gomain(x)
	{
		...
		if(x == 'all')
		{
			window.location.href= '<?php echo base_url();?>cocktail_suggestion/list_cocktail_suggestion/<?php echo $bars_id ?>';
		}
...
<div class="page_content">
			...
			<div class="container_fluid">
				...
				<div class="row_fluid"> 
					<h3 class="page_title">Cocktail Suggestion Directory</h3>
				...
				<div class="row_fluid"> 
					<div class="portlet blue">
						<div class="portlet-title">
							<div class="caption fl_left">Cocktail Suggestion  Directory</div>
							<div class="fl_right">
					...
					echo form_open('cocktail_suggestion/search_list_cocktail_suggestion/'.$limit,$attributes);?>
					<div class="fl_left">
					...
					<input type="hidden" name="bars_id" id="bars_id" value="<?php echo $bars_id; ?>" />
                            ...
                            <input type="button" name="refresh" id="refresh" value="Refresh" class="btn blue  fl_left mar10" onclick="document.location.href = '<?php echo site_url("cocktail_suggestion/list_cocktail_suggestion/20/".$bars_id); ?>'" /> 
								</form>
								</div>
					<div class="fl_right">	
					...
					<div class="clear"></div>
									...
									<div class="clear"></div>
								<form name="frm_listlogin" id="frm_listlogin" action="<?php echo base_url();?>cocktail_suggestion/action_cocktail_suggestion" method="post">
								...
								if($result)
								{
									...
									<td  style="width: 30px;"><input type="checkbox" name="chk[]" value="<?php echo $row->cocktail_id ?>" class="chk"  /></td>
														<td class="sorting_1"><?php echo $row->cocktail_name; if($row->states == 'unread')
														{?>
											...
											<?php } else { ?>
												...
												<?php }?>
														...
														<td class="center">
															<a class="btn blue mini" href="<?php echo site_url("cocktail_suggestion/viewcoktail/".$row->cocktail_id); ?>" title="View">View</a>
														</td>
														<td style="text-align: center;">
														<?php 
							//FUTURE 
							// 	echo anchor('cocktail_suggestion/edit_cocktail_suggestion/'.$row->cocktail_suggestion_id.'/'.$redirect_page.'/'.$option.'/'.$keyword.'/'.$limit.'/'.$offset,'<i class="comon_icon edit_icon"></i>','class="table_icon btn blue" id="cocktail_suggestion_'.$row->cocktail_suggestion_id.'" title="Edit cocktail_suggestion"'); 
						?>
														<!--Futture enhancement															
