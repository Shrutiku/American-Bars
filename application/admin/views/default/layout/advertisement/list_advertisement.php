<script type="text/javascript" language="javascript">
	...
	{
		var ans = confirm("Are you sure, you want to delete Advertisement?");
		if(ans)
		{
			location.href = "<?php echo base_url(); ?>advertisement/delete_advertisement/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+limit+"/"+offset;
		}else{
			return false;
		}
	}
	...
	function getlimit(limit)
	{
		...
		{
			window.location.href='<?php echo base_url();?>advertisement/list_advertisement/'+limit+'/';
		}
	...
	function getsearchlimit(limit)
	{
		...
		else
		{
			...
			window.location.href='<?php echo base_url();?>advertisement/search_list_advertisement/'+limit+'/<?php echo $option.'/'.$keyword; ?>';
		}
	...
	function gomain(x)
	{
		...
		if(x == 'all')
		{
			window.location.href= '<?php echo base_url();?>advertisement/list_advertisement';
		}
...
<div class="page_content">
			...
			<div class="container_fluid">
				...
				<div class="row_fluid"> 
					<h3 class="page_title">Advertisement Listing</h3>
    ...
    <?php } ?>
				<div class="row_fluid"> 
					...
					echo form_open('advertisement/search_list_advertisement/'.$limit,$attributes);?>
					<div class="fl_left">
					...
					<span class="sspan fl_left ">&nbsp;&nbsp;&nbsp;Search By</span>&nbsp;
                            <select name="option" id="option" onchange="gomain(this.value)"  class="m_wrap fl_left mar0" style="padding:6px;">
                                ...
                                <option value="advertisement_title" <?php if($option=='advertisement_title'){?> selected="selected"<?php }?>>Title</option>  
							...
							<input type="button" name="refresh" id="refresh" value="Refresh" class="btn blue  fl_left mar10" onclick="document.location.href = '<?php echo site_url("advertisement/list_advertisement"); ?>'" />  
								</form>
								</div>
									<div class="fl_right">
										
										
					<?php echo anchor('advertisement/add_city_advertisement','Add City/Zip AD', 'class="btn green  fl_left mar_r_5" id="addadvertisement"'); ?>					
					<?php echo anchor('advertisement/add_advertisement','Add New', 'class="btn blue  fl_left mar_r_5" id="addadvertisement"'); ?>
					...
					<div class="clear"></div>		
									...
									<div class="clear"></div>
			<form name="frm_listlogin" id="frm_listlogin" action="<?php echo base_url();?>advertisement/action_advertisement" method="post">
	<input type="hidden" name="offset" id="offset" value="<?php echo $offset; ?>" />
				   ...
				   <div class="scroll-pane horizontal-only" >
									<table class="table border" >
												...
												<th class="sorting" style="width: 18%;">size</th>
												<th class="sorting" style="width: 12%;">Click Hits / Visits</th>
												<th class="sorting" style="width: 12%;">Used Clicks / Visits</th>
												<th class="sorting" style="width: 18%;">Pages</th>
												<th class="sorting" style="width: 18%;">City / Zip</th>
								...
								if($result)
								{
									...
									<td  style="width: 30px;"><input type="checkbox" name="chk[]" value="<?php echo $row->advertisement_id ?>" class="chk"  /></td>
														<td class=" sorting_1"><?php echo $row->advertisement_title; ?></td>														
														<td><?php echo $row->size; ?></td>
														<td><?php echo $row->type=='click' ? $row->number_click:'0'; ?> / <?php echo $row->type=='visit' ? $row->number_visit:'0'; ?></td>
														<td><?php echo $row->type=='click' ? $row->total_click:'0'; ?> / <?php echo $row->type=='visit' ? $row->total_visit:'0'; ?></td>
														<td><?php echo $row->pages; ?></td>
														<td><?php $g = explode( ',' , $row->city_zip);
														     $g = array_filter($g);
														    if($g)
															{
																foreach($g as $rr)
														...
														<!-- <td class="center"><?php if($this->session->userdata('advertisement_type')=='1'){echo anchor('rights/assign_rights/'.$row->advertisement_id.'/'.$limit.'/'.$offset,'<i class="halflings-icon ok"></i>'); }else{ echo "N/A";} ?></td>
														<td>
														<a class="table_icon btn green" href="<?php echo site_url('advertisement/list_logactivity/'.$row->advertisement_id);?>">
														...
														<?php 
							                                    if($row->city_zip=='')
																{
																	echo anchor('advertisement/edit_advertisement/'.$row->advertisement_id.'/'.$redirect_page.'/'.$option.'/'.$keyword.'/'.$limit.'/'.$offset,'<i class="comon_icon edit_icon"></i>','class="table_icon btn blue" id="advertisement_'.$row->advertisement_id.'" title="Edit Admin"');
																}
																else {
																	echo anchor('advertisement/edit_city_advertisement/'.$row->advertisement_id.'/'.$redirect_page.'/'.$option.'/'.$keyword.'/'.$limit.'/'.$offset,'<i class="comon_icon edit_icon"></i>','class="table_icon btn blue" id="advertisement_'.$row->advertisement_id.'" title="Edit Admin"');
																}
																 
														?>
														
														<a class="table_icon btn red" href="javascript://" onClick="delete_rec('<?php echo $row->advertisement_id; ?>','<?php echo $redirect_page;?>','<?php echo $option?>','<?php echo $keyword?>','<?php echo $limit?>','<?php echo $offset; ?>')" title="Delete"><i class="comon_icon delete_icon"></i></a>
														</td>
													</tr>
								<?php $i++;} }else{ ?>
