...
$(document).ready(function(){		
	...
	{
		var ans = confirm("Are you sure, you want to delete term?");
		if(ans)
		{
			location.href = "<?php echo base_url(); ?>term/delete_term/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+limit+"/"+offset;
		}else{
			return false;
		}
	}
	...
	function getlimit(limit)
	{
		...
		{
			window.location.href='<?php echo base_url();?>term/list_term/'+limit+'/';
		}
	...
	function getsearchlimit(limit)
	{
		...
		else
		{
			...
			window.location.href='<?php echo base_url();?>term/search_list_term/'+limit+'/<?php echo $option.'/'.$keyword; ?>';
		}
	...
	function gomain(x)
	{
		...
		if(x == 'all')
		{
			window.location.href= '<?php echo base_url();?>term/list_term/';
		}
...
<div class="page_content">
			...
			<div class="container_fluid">
				...
				<div class="row_fluid"> 
					<h3 class="page_title">Dive Bar Term</h3>
				...
				<div class="row_fluid"> 
					<div class="portlet blue">
						<div class="portlet-title">
							<div class="caption fl_left"> Dive Bar Term </div>
							<div class="fl_right">
					...
					echo form_open('term/search_list_term/'.$limit,$attributes);?>
					<div class="fl_left">
					...
					<span class="sspan fl_left ">&nbsp;&nbsp;&nbsp;Search By</span>&nbsp;
                            <select name="option" id="option" onchange="gomain(this.value)"  class="m_wrap fl_left mar0" style="padding:6px;">
                                ...
                                <option value="term_title" <?php if($option=='term_title'){?> selected="selected"<?php }?>>Term Title</option>  
<?php /*?>                                <option value="type" <?php if($option=='type'){?> selected="selected"<?php }?>>Type</option>                 
                            ...
                            <input type="submit" name="submit" id="submit" value="Search" class="btn blue  fl_left mar10" /> 
                             ...
                             <input type="button" name="refresh" id="refresh" value="Refresh" class="btn blue  fl_left mar10" onclick="document.location.href = '<?php echo site_url("term"); ?>'" /> 
								</form>
								</div>
									<div class="fl_right">
					...
					<?php echo anchor('term/add_term','Add New', 'class="btn blue  fl_left mar_r_5" id="addterm"'); ?>
					...
					<div class="clear"></div>		
									...
									<div class="clear"></div>
					<form name="frm_listlogin" id="frm_listlogin" action="<?php echo base_url();?>term/action_term" method="post">
				   ...
				   <input type="hidden" name="redirect_page" id="redirect_page" value="<?php echo $redirect_page;?>"/>
								<div class="scroll-pane horizontal-only" >	<table class="table border" >
												...
												<th class="sorting_disabled" style="width: 6%;">Term Title</th>
												<th class="sorting" style="width: 5%;">Description</th>
												<th class="sorting" style="width: 5%;">Term Usage</th>
								...
								if($result)
								{
									...
									<td  style="width: 30px;"><input type="checkbox" name="chk[]" value="<?php echo $row->term_id ?>" class="chk"  /></td>
														<td class=" sorting_1"><?php echo $row->term_title; ?></td>
														<td><?php echo $row->description; ?></td>
														<td><?php echo $row->term_usage; ?></td>										
														<td class="center">
														...
														<?php 
							//FUTURE 
							// 	echo anchor('term/edit_term/'.$row->term_id.'/'.$redirect_page.'/'.$option.'/'.$keyword.'/'.$limit.'/'.$offset,'<i class="comon_icon edit_icon"></i>','class="table_icon btn blue" id="term_'.$row->term_id.'" title="Edit term"'); 
						?>
														<!--Futture enhancement															
															<a class="btn green table_icon" href="javascript:;"><i class="comon_icon view_icon"></i></a>-->
														<?php echo  anchor('term/edit_term/'.$row->term_id.'/'.$redirect_page.'/'.$option.'/'.$keyword.'/'.$limit.'/'.$offset,'<i class="comon_icon edit_icon"></i>','class="table_icon btn blue" id="term_'.$row->term_id.'" title="Edit term"');  ?>
														<a class="table_icon btn red" href="javascript://" onClick="delete_rec('<?php echo $row->term_id; ?>','<?php echo $redirect_page;?>','<?php echo $option?>','<?php echo $keyword?>','<?php echo $limit?>','<?php echo $offset; ?>')" title="Delete"><i class="comon_icon delete_icon"></i></a>
														</td>
													</tr>
								<?php $i++;} }else{ ?>
