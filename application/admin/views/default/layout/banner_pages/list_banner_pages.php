<script type="text/javascript" language="javascript">
	...
	{
		var ans = confirm("Are you sure, you want to delete banner_pages?");
		if(ans)
		{
			location.href = "<?php echo base_url(); ?>banner_pages/delete_banner_pages/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+limit+"/"+offset;
		}else{
			return false;
		}
	}
	...
	function getlimit(limit)
	{
		...
		{
			window.location.href='<?php echo base_url();?>banner_pages/list_banner_pages/'+limit+'/';
		}
	...
	function getsearchlimit(limit)
	{
		...
		else
		{
			...
			window.location.href='<?php echo base_url();?>banner_pages/search_list_banner_pages/'+limit+'/<?php echo $option.'/'.$keyword; ?>';
		}
	...
	function gomain(x)
	{
		...
		if(x == 'all')
		{
			window.location.href= '<?php echo base_url();?>banner_pages/list_banner_pages';
		}
...
<div class="page_content">
			...
			<div class="container_fluid">
				...
				<div class="row_fluid"> 
					<h3 class="page_title">Banner Pages Listing</h3>
    ...
    <?php } ?>
				<div class="row_fluid"> 
					...
					echo form_open('banner_pages/search_list_banner_pages/'.$limit,$attributes);?>
					<div class="fl_left">
					...
					<span class="sspan fl_left ">&nbsp;&nbsp;&nbsp;Search By</span>&nbsp;
                            <select name="option" id="option" onchange="gomain(this.value)"  class="m_wrap fl_left mar0" style="padding:6px;">
                                ...
                                <option value="banner_pages_title" <?php if($option=='banner_pages_title'){?> selected="selected"<?php }?>>Title</option>  
							...
							<input type="button" name="refresh" id="refresh" value="Refresh" class="btn blue  fl_left mar10" onclick="document.location.href = '<?php echo site_url("banner_pages/list_banner_pages"); ?>'" />  
								</form>
								</div>
									<div class="fl_right">
										
										
					<?php echo anchor('banner_pages/add_banner_pages','Add New', 'class="btn blue  fl_left mar_r_5" id="addbanner_pages"'); ?>
					...
					<div class="clear"></div>		
									...
									<div class="clear"></div>
			<form name="frm_listlogin" id="frm_listlogin" action="<?php echo base_url();?>banner_pages/action_banner_pages" method="post">
	<input type="hidden" name="offset" id="offset" value="<?php echo $offset; ?>" />
				   ...
				   <div class="scroll-pane horizontal-only" >
								...
								if($result)
								{
									...
									<td  style="width: 30px;"><input type="checkbox" name="chk[]" value="<?php echo $row->banner_pages_id ?>" class="chk"  /></td>
														<td class=" sorting_1"><?php echo $row->banner_pages_title; ?></td>														
														...
														<!-- <td class="center"><?php if($this->session->userdata('banner_pages_type')=='1'){echo anchor('rights/assign_rights/'.$row->banner_pages_id.'/'.$limit.'/'.$offset,'<i class="halflings-icon ok"></i>'); }else{ echo "N/A";} ?></td>
														<td>
														<a class="table_icon btn green" href="<?php echo site_url('banner_pages/list_logactivity/'.$row->banner_pages_id);?>">
														...
														<?php 
							                                    if($row->city_zip=='')
																{
																	echo anchor('banner_pages/edit_banner_pages/'.$row->banner_pages_id.'/'.$redirect_page.'/'.$option.'/'.$keyword.'/'.$limit.'/'.$offset,'<i class="comon_icon edit_icon"></i>','class="table_icon btn blue" id="banner_pages_'.$row->banner_pages_id.'" title="Edit Admin"');
																}
																else {
																	echo anchor('banner_pages/edit_city_banner_pages/'.$row->banner_pages_id.'/'.$redirect_page.'/'.$option.'/'.$keyword.'/'.$limit.'/'.$offset,'<i class="comon_icon edit_icon"></i>','class="table_icon btn blue" id="banner_pages_'.$row->banner_pages_id.'" title="Edit Admin"');
																}
																 
														?>
														
														<a class="table_icon btn red" href="javascript://" onClick="delete_rec('<?php echo $row->banner_pages_id; ?>','<?php echo $redirect_page;?>','<?php echo $option?>','<?php echo $keyword?>','<?php echo $limit?>','<?php echo $offset; ?>')" title="Delete"><i class="comon_icon delete_icon"></i></a>
														</td>
													</tr>
								<?php $i++;} }else{ ?>
