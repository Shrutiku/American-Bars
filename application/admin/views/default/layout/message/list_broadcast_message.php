...
<script type="text/javascript" language="javascript">
	function delete_rec(id,redirectpage,option,keyword,limit,offset) {
		var ans = confirm("Are you sure, you want to delete boradcast message?");
		if(ans) {
			location.href = "<?php echo base_url(); ?>message/delete_boradcast_message/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+limit+"/"+offset;
		}else{
			return false;
		}
	}
	...
	function getlimit(limit) {
		...
		if(limit=='0'){
			...
			window.location.href='<?php echo base_url();?>message/list_broadcast_message/'+limit+'/';
		}
	}	
	...
	function getsearchlimit(limit) {
		if(limit=='0') {
			...
			window.location.href='<?php echo base_url();?>message/search_list_broadcast_message/'+limit+'/<?php echo $option.'/'.$keyword; ?>';
		}
	...
	function gomain(x) {
		if(x == 'all')
		{
			window.location.href= '<?php echo base_url();?>message/list_broadcast_message';
		}
	}
...
$(document).ready(function(){
	...
	$('.view_message').click(function(){
		...
		$(this).colorbox({href:"<?php echo base_url();?>message/get_broadcast_message_view?message_id="+msg_id+"&msg_type=inbox", title:'View Message Detail'});
		 
		/*$.ajax({url:"<?php echo base_url();?>message/get_message_view", data: {'message_id':msg_id},success:function(result){
			alert(result);
		}});*/
	});
});
				...
				<div class="row_fluid"> 
					...
					<?php 
						...
						if($msg != ""){
							...
							if($msg == "send"){ $error = MESSAGE_SEND;}
					?>
				...
				<div class="row_fluid"> 
					<div class="portlet blue">
						<div class="portlet-title">
							<div class="caption fl_left">Inbox</div>
							<div class="fl_right">
						...
						<div class="portlet-body form">
							...
							<?php /*?><div class="fl_left">
								 <?php			 
									...
									echo form_open('message/search_list_broadcast_message/'.$limit,$attributes);?>
									<div class="fl_left">
							...
							<div class="fl_right">
									...
									<?php echo anchor('message/add_broadcast_message','Compose Message', 'class="btn blue  fl_left mar_r_5" id="addmessage"'); ?>
									...
									<div class="clear"></div>
								<form name="frm_listlogin" id="frm_listlogin" action="<?php echo base_url();?>message/action_broadcast_message" method="post">
								...
								<div class="scroll-pane horizontal-only" >	
									<table class="table border" >
										<thead>
											...
											<th class="sorting" style="width: 5%;">Total Recipients</th>
										...
										<tbody role="alert" aria-live="polite" aria-relevant="all">	
											...
											if($result)
											{
												...
												<tr class="<?php echo $cls ?>">
													<td  style="width: 30px;"><input type="checkbox" name="chk[]" value="<?php echo $row->number; ?>" class="chk"  /></td>
													...
													<td>
														<?php echo getcountrecipient($row->number); 
														?></td>
													<td>
														...
														<a class="table_icon btn red" href="javascript://" onClick="delete_rec('<?php echo $row->number; ?>','<?php echo $redirect_page;?>','<?php echo $option?>','<?php echo $keyword; ?>','<?php echo $limit; ?>','<?php echo $offset; ?>')" title="Delete"><i class="comon_icon delete_icon"></i></a>
