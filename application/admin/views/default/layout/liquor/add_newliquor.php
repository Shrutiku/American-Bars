...
$(document).ready(function(){	
	...
	$('.tags').tagsInput({
		autocomplete_url:'<?php echo site_url('liquor/getallliquorbybar/');?>',
		itemValue: 'value',
		itemText: 'text',
		autocomplete:{
		  ...
		  var t = <?php echo $bars_id; ?>;
			  $.ajax({
				 url: "<?php echo site_url('liquor/getallliquorbybar');?>",
				 dataType: "json",
				 data: {
				   	utype : t,
					em: request.term,
...
<div class="page_content">
			<div class="container_fluid">
				...
				echo form_open_multipart('liquor/add_newliquor/'.$bars_id,$attributes);
			  ?>
			<div class="control_group">
										<label class="control_label">Select liquor Name :<i style="color: #7D2A1C;">*</i></label>
										<div class="controls">
											<!-- <input type="text" name="liquor_id" id="liquor_id" class="m_wrap wid360 tags" /> -->
											<!-- <label for="liquor_id" generated="true" class="error" style="display: none;">This field is required.</label> -->
											<select data-placeholder="Add liquor" name="liquor_id[]" id="liquor_id" style="width:600px; height: 100px;" class="m_wrap wid360 chosen" multiple="multiple" tabindex="6">
												<?php if($getallliquor)
												      { foreach($getallliquor as $rows)
														  {
														  	$chckexist = checkliquorbarexist($rows->liquor_id,$bars_id);
															if($chckexist==0)
														    {
														  	?>
														     <option value="<?php echo $rows->liquor_id; ?>"><?php echo $rows->liquor_title; ?></option>
					...
					<?php if($redirect_page == 'list_liquor')
						{
							?>
						<input type="button" name="Cancel" value="Cancel" class="btn red  fl_left" onClick="location.href='<?php echo base_url(); ?>liquor/<?php echo $redirect_page.'/'.$limit.'/'.$bars_id.'/'.$offset?>'" />
						<?php } else
						{?>
						<input type="button" name="Cancel" value="Cancel" class="btn red  fl_left" onClick="location.href='<?php echo base_url(); ?>liquor/<?php echo $redirect_page.'/'.$limit.'/'.$bars_id.'/'.$option.'/'.$keyword.'/'.$offset?>'" />
