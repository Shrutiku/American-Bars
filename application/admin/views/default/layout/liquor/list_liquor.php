...
$(document).ready(function(){
	...
	function delete_rec(id,redirectpage,option,keyword,bar_id,limit,offset)
	{
		...
		if(ans)
		{
			location.href = "<?php echo base_url(); ?>liquor/delete_liquor/"+id+"/"+redirectpage+"/"+option+"/"+keyword+"/"+bar_id+"/"+limit+"/"+offset;
		}else{
			return false;
		}
	}
	...
	function getlimit(limit)
	{
		...
		{
		 window.location.href= '<?php echo base_url();?>liquor/list_liquor/'+limit+'/<?php echo $bars_id ?>';
		}
	...
	function getsearchlimit(limit)
	{
		...
		else
		{
			...
			window.location.href='<?php echo base_url();?>liquor/search_list_liquor/'+limit+'/<?php echo $option.'/'.$keyword; ?>';
		}
...
<div class="page_content">
			...
			<div class="container_fluid">
				...
				<div class="row_fluid"> 
					<h3 class="page_title">Liquor Listing</h3>
   ...
   if($er!='')
    ...
    <?php } ?>
				<div class="row_fluid"> 
					<div class="portlet blue">
						<div class="portlet-title">
							<div class="caption fl_left">Liquor Listing</div>
							<div class="fl_right">
					...
					echo form_open('liquor/search_list_liquor/'.$limit,$attributes);?>
					<div class="fl_left">
					...
					<span class="sspan fl_left ">&nbsp;&nbsp;&nbsp;Search By</span>&nbsp;
                            ...
                            <input type="submit" name="submit" id="submit" value="Search" class="btn blue  fl_left mar10" /> 
                             ...
                             <input type="button" name="refresh" id="refresh" value="Refresh" class="btn blue  fl_left mar10" onclick="document.location.href = '<?php echo site_url("liquor/list_liquor/".$limit.'/'.$bars_id); ?>'" /> 
								</form>
								</div>
									<div class="fl_right">
										
					<?php  if($bars_id!='' && $bars_id!=0){?>
					<?php echo anchor('liquor/add_newliquor/'.$bars_id,'Add Liquor', 'class="btn blue  fl_left mar_r_5" id="addbeer"'); ?>
				<?php } else {?>						
										
					<?php echo anchor('liquor/add_liquor/'.$bars_id,'Add New', 'class="btn blue  fl_left mar_r_5" id="addliquor"'); ?>
				...
				<?php if($this->session->userdata('admin_type')==1){?>
					...
					<?php } ?>	
					<?php echo anchor('liquor/import','Import xls', 'class="btn blue  fl_left mar_r_5" id="addbeer"'); ?>
					<a href="<?php echo front_base_url().'/upload/Liquor.xls';?>" class='btn purple fl_left mar_r_5'>Demo xls</a>
				<?php } ?>
					...
					<div class="clear"></div>		
									...
									<div class="clear"></div>
			<form name="frm_listlogin" id="frm_listlogin" action="<?php echo base_url();?>liquor/action_liquor" method="post">
							...
							<div class="scroll-pane horizontal-only" >		<table class="table border" >
								...
								if($result)
								{
									...
									<td  style="width: 30px;"><input type="checkbox" name="chk[]" value="<?php echo $row->liquor_id ?>" class="chk"  /></td>
														...
														<td class="center">
															<a class="table_icon btn blue" href="<?php echo site_url("liquor/view_liquor_comment/".$row->liquor_id); ?>" title="View"><i class="comon_icon view_icon"></i></a>
														...
														<?php echo  anchor('liquor/edit_liquor/'.$row->liquor_id.'/'.$redirect_page.'/'.$option.'/'.$keyword.'/'.$limit.'/'.$offset."/".$bars_id,'<i class="comon_icon edit_icon"></i>','class="table_icon btn blue" id="liquor_'.$row->liquor_id.'" title="Edit liquor"');  ?>
