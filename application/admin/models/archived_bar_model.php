...
class Archived_bar_model extends CI_Model {
...
// 			
	...
	function get_total_archived_bar_count($bar_type = '')
	{
	...
	function get_archived_bar_result($offset=0, $limit=0,$bar_type = '')
	{
	...
	function get_total_search_archived_bar_count($bar_type='',$option='',$keyword='')
	{
...
// 				
	...
	function get_search_archived_bar_result($bar_type,$option,$keyword,$offset, $limit)
	{
	...
	if(isset($data->sheets[$i]['cells']) && count($data->sheets[$i]['cells'])>0) // checking sheet not empty
	{
		for($j=1;$j<=count($data->sheets[$i]['cells']);$j++) // loop used to get each row of the sheet
		{
			...
			$arr=array(
										...
										'bar_slug'=>$slug,
										'linkurl'=>'http://sandbox.americanbars.com/bar/details/'.$slug,
										'status'=>'Active',
										'lat' => $getlat['lat'],
										'lang' => $getlat['lng'],
										'bar_type'=>'half_mug',
