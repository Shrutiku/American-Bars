...
class Album_model extends CI_Model {
	...
	{
		$query = $this->db->get_where('album',array('bar_gallery_id'=>$id));
		return $query->row_array();
	}	
	
	function getOneImageGallery($id)
	{
		$query = $this->db->get_where('album_images',array('bar_gallery_id'=>$id));
		return $query->row();
	}
	...
	{
		$this->db->select('*,album.status as status');
		$this->db->from('album');
		$this->db->join('user_master','user_master.user_id=album.bar_id','left');
		$this->db->where('album.bar_id',$bar_id);
	...
	function get_search_gallery_result($option='',$keyword='',$offset =0, $limit = 0,$bar_id = 0)
	{
		...
		$this->db->select('*');
		$this->db->from('album');
		$this->db->join('user_master','user_master.user_id=album.bar_id');
		if($bar_id > 0 && is_numeric($bar_id))
		{
			$this->db->where("album.bar_id",$bar_id);
		}
