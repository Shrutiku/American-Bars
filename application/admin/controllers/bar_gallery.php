<?php
class Bar_gallery extends  CI_Controller {
	function Bar_gallery()
	{
		parent::__construct();	
		$this->load->model('bar_gallery_model');	
	...
	function index()
	{
		...
		redirect('bar_gallery/list_gallery');	
	}
	 ...
	 * 
	 */
	function list_gallery($limit=20,$bar_id ='',$offset=0,$msg='') {
		...
		if($bar_id==0 || $bar_id=='')
		{
			redirect('bar/list_bar/full_mug');
		}
		...
		*/
		...
		$config['base_url'] = base_url().'bar_gallery/list_gallery/'.$limit.'/'.$bar_id. "/";
		$config['total_rows'] = $this->bar_gallery_model->get_total_gallery_count($bar_id);
	
		$config['per_page'] = $limit;		
		$this->pagination->initialize($config);		
		$data['page_link'] = $this->pagination->create_links();
		
		$data['result'] = $this->bar_gallery_model->get_gallery_result($offset,$limit,$bar_id);
		$data['msg'] = $msg;
		...
		$this->template->write_view('center',$theme .'/layout/bar_gallery/list_gallery',$data,TRUE);
	...
	function search_list_gallery($limit=20,$option='',$keyword='',$bar_id = 0,$offset=0,$msg='')
	{
		...
		$config['base_url'] = base_url().'bar_gallery/'.$redirect_page.'/'.$limit.'/'.$option.'/'.$keyword.'/'.$bar_id;
		$config['total_rows'] = $this->bar_gallery_model->get_total_search_gallery_count($option,$keyword,$bar_id);
		$config['per_page'] = $limit;		
		$this->pagination->initialize($config);		
		$data['page_link'] = $this->pagination->create_links();
		
		$data['result'] = $this->bar_gallery_model->get_search_gallery_result($option,$keyword,$offset, $limit,$bar_id);
	...
	function add_gallery($bar_id = 0,$limit=20)
	{
		...
		*/
		...
		$data["image"] = $this->input->post('prev_photo_image');
		...
		// $data['cities']=get_all_city_by_state_id($this->input->post('state'));
			...
			$this->template->write_view('header',$theme .'/layout/common/header',$data,TRUE);
			 $this->template->write_view('left',$theme .'/layout/common/sidebar',$data,TRUE);
			$this->template->write_view('center',$theme .'/layout/bar_gallery/add_gallery',$data,TRUE);
			$this->template->write_view('footer',$theme .'/layout/common/footer',$data,TRUE);
			$this->template->render();
		}else{
			...
			{	
				...
				$this->bar_gallery_model->gallery_update($_POST);
				$msg = "update";
			}else{
				$this->bar_gallery_model->gallery_insert($_POST);			
				$msg = "insert";
			}
			...
			{
				redirect('bar_gallery/'.$redirect_page.'/'.$limit.'/'.$this->input->post("bar_id").'/'.$offset.'/'.$msg);
			}
			
			else
			{
				redirect('bar_gallery/'.$redirect_page.'/'.$limit.'/'.$option.'/'.$keyword.'/'.$this->input->post("bar_id").'/'.$offset.'/'.$msg);
			}
		}				
	}
	...
	function removeImageAjax($id=0)
	{
		...
		$oneImage=$this->bar_gallery_model->getOneImageGallery($id);
		if($oneImage!='')
		{
	...
	function edit_gallery($id=0,$redirect_page='',$option='',$keyword='',$limit=0,$offset=0,$bar_id = 0)
	{
		...
		*/
		...
		$one_gallery = $this->bar_gallery_model->get_one_gallery($id);
		$data["error"] = "";
		$data["limit"]=$limit;
		$data["offset"]=$offset;
		$data["option"]=$option;
		...
		$data['imageGallery']=$this->bar_gallery_model->getImageGallery($id);
	...
	function delete_gallery($id=0,$redirect_page='',$option='',$keyword='',$bar_id = 0,$limit=20,$offset=0)
	{
		...
		*/
		...
		if($redirect_page == 'list_gallery')
		{
			...
			redirect('bar_gallery/'.$redirect_page.'/'.$limit.'/'.$bar_id.'/'.$offset.'/delete');
		}
		else
		{
		redirect('bar_gallery/'.$redirect_page.'/'.$limit.'/'.$bar_id.'/'.$option.'/'.$keyword.'/'.$offset.'/delete');
	...
	function action_gallery()
	{
		...
		*/
		...
		if($action=='delete')
		{
			...
			{
				redirect('bar_gallery/'.$redirect_page.'/'.$limit.'/'.$option.'/'.$keyword.'/'.$bar_id.'/'.$offset.'/delete');
		...
		if($action=='active')
		{
			...
			if($redirect_page == 'list_gallery')
			{
				redirect('bar_gallery/'.$redirect_page.'/'.$limit.'/'.$bar_id.'/'.$offset.'/active');
			}
			
			else
			{
				redirect('bar_gallery/'.$redirect_page.'/'.$limit.'/'.$option.'/'.$keyword.'/'.$bar_id.'/'.$offset.'/active');
		...
		if($action=='inactive')
		{
			...
			if($redirect_page == 'list_gallery')
			{
				redirect('bar_gallery/'.$redirect_page.'/'.$limit.'/'.$bar_id.'/'.$offset.'/inactive');
			}
			
			else
			{
				redirect('bar_gallery/'.$redirect_page.'/'.$limit.'/'.$option.'/'.$keyword.'/'.$bar_id.'/'.$offset.'/inactive');
	...
	function view($id=0,$msg=''){
		...
		//echo $id; die;
		$data["one_gallery_detail"] = $this->bar_gallery_model->get_one_gallery($id);
	   ...
	   $data["msg"] = $msg;
		...
		$data['reply'] = $this->bar_gallery_model->get_gallery_comment_result($id);
		
		
	
		
		if($_POST){
			$this->bar_gallery_model->reply_insert($_POST);			
			$msg = "insert"; 
			//echo 'bar/view/'.$limit.'/'.$offset; die;
			 redirect('bar_gallery/view/'.$id.'/'.$msg);
		...
		$this->template->write_view('header_menu',$theme .'/layout/common/header',$data,TRUE);
		$this->template->write_view('left',$theme .'/layout/common/sidebar',$data,TRUE);
		$this->template->write_view('center',$theme .'/layout/bar_gallery/view',$data,TRUE);
		$this->template->write_view('footer',$theme .'/layout/common/footer',$data,TRUE);
		$this->template->render();
	...
	function delete_comment($gallery_id = 0, $gallery_comment_id = 0 )
	{
		...
		redirect ("bar_gallery/view/".$gallery_id."/delete");
	}
	function removeimage($gallery_id,$image,$limit,$offset,$redirect_page,$option,$keyword)
	{
		...
		redirect('bar_gallery/edit_gallery/'.$gallery_id.'/'.$redirect_page.'/'.$limit);
	}
}
?>