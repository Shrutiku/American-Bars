...
class Adborder extends CI_Controller
{
	...
	function Adborder()
	{
		...
		parent::__construct();	
		$this->load->model('adborder_model');
	...
	function orderlist($limit=10,$offset=0,$msg='')
	{
		...
		$check_rights=get_rights('adb_orderlist');
		...
		$config['base_url'] = site_url('adborder/orderlist/'.$limit);
		$config['div'] = '#content';
		//load total row
		$config['total_rows'] = $this->adborder_model->getTotalOrder();
	    $config['per_page'] = $limit;
		$this->pagination->initialize($config);
		
		$data['page_link']=$this->pagination->create_links();	
	    $data['result']=$this->adborder_model->getOrderResult($limit, $offset);
		...
		if($this->input->is_ajax_request()){
			echo $this->load->view($theme .'/layout/adborder/orderlist_ajax',$data,TRUE);die;
		}
		else
		{
			...
			$this->template->write_view('center',$theme .'/layout/adborder/orderlist',$data,TRUE);
	...
	function searchOrderlist($limit=10,$option='1V1',$keyword='1V1',$from_date='1V1',$to_date='1V1',$offset=0,$msg='')
	{
		...
		$check_rights=get_rights('adb_searchOrderlist');
		...
		else
		{
	     ...
	     $config["total_rows"] = $this->adborder_model->get_total_search_order_count($option,$keyword,$keyword_date,$from_date,$to_date);
	     $config['div'] = '#content';
	     $config["per_page"] = $limit;		
	     $this->pagination->initialize($config);	
	     ...
	     $data["result"] = $this->adborder_model->get_search_order_result($option,$keyword,$offset, $limit,$keyword_date,$from_date,$to_date);
	     if($data['result']==0)
		 {	
			if($offset>=$limit)
			{
			...
			if($offset>=0)
			{
				redirect('adborder/searchOrderlist/'.$limit.'/'.$option.'/'.$keyword.'/'.$offset.'/'.$msg);
			}else{
				$offset=0;
			}
				$data['offset']=$offset;
	...
	{   //$limit="20";
	$check_rights=get_rights('adb_deleteOrder');
		...
		{
			redirect('adborder/'.$redirect_page.'/'.$limit.'/'.$offset.'/delete');
		}
		else
		{
			redirect('adborder/'.$redirect_page.'/'.$limit.'/'.$bars_id.'/'.$option.'/'.$keyword.'/'.$from_date.'/'.$to_date.'/'.$offset.'/delete');
	...
	{
		$check_rights=get_rights('adb_actionOrder');
    ...
    */
    function DowloadCsv()
	{
		$result=$this->adborder_model->downloadOrder();
		$filename ="Order.csv";
		$this->load->dbutil();
		$delimiter = ",";
		$newline = "\r\n";
    ...
    */
	function orderDetails($id=0,$limit=10,$option='1V1',$keyword='1V1',$offset=0)
	{
		...
		$check_rights=get_rights('adb_orderDetails');
		...
		$one_order = $this->adborder_model->get_one_order($id);
		$one_order_new = $this->adborder_model->get_one_order_info($id);
		...
		$this->template->write_view('header',$theme .'/layout/common/header',$data,TRUE);
		$this->template->write_view('left',$theme .'/layout/common/sidebar',$data,TRUE);
		$this->template->write_view('center',$theme .'/layout/adborder/orderdetails',$data,TRUE);
		$this->template->write_view('footer',$theme .'/layout/common/footer',$data,TRUE);
		$this->template->render();
	}
    ...
    */
    function statusChange($order_id, $order_status,$limit=10,$offset=0,$msg='update')
    {
		...
		$data['page_link']=$this->pagination->create_links();	
	    $data['result']=$this->adborder_model->getOrderResult($limit, $offset);
		$this->adborder_model->changeOrderStatus($order_id,$order_status);
		echo 'done';die;
	}
	//end of statusChange
	/*
    ...
    */
	function searchOrderlistDate($limit='10',$option='1V1', $offset=0, $msg='', $keyword='')
	{
		...
		$check_rights=get_rights('adb_searchOrderlistDate');
	    ...
	    $config["total_rows"] = $this->adborder_model->get_total_search_orderdate_count($option,$from_date,$to_date);
	    $config['div'] = '#content';
	    $config["per_page"] = $limit;		
	    $this->pagination->initialize($config);	
	    $data["page_link"] = $this->pagination->create_links();
	    $data["result"] = $this->adborder_model->get_search_orderdate_result($option,$from_date,$offset, $limit,$to_date);
	    if($data['result']==0)
		{	
			if($offset>=$limit)
			{
			...
			if($offset>=0)
			{
				redirect('adborder/searchOrderlistDate/'.$limit.'/'.$offset.'/'.$msg);
			}else{
				$offset=0;
			}
				$data['offset']=$offset;
		...
		else
		{
			...
			$this->template->write_view('content_center',$theme .'/layout/adborder/orderlist',$data,TRUE);
			$this->template->write_view('footer',$theme .'/layout/common/footer',$data,TRUE);	
			$this->template->render();
		}
	...
	function print_order($id=0)
	{
			...
			if(!check_admin_authentication())
		{
			...
			$check_rights=get_rights('adb_print_order');
		...
		$this->load->view($theme .'/layout/adborder/printorder',$data);
		//$this->template->write_view('footer',$theme .'/layout/common/footer',$data,TRUE);
		//$this->template->render();
	}	
