if ( typeof jwplayer == "undefined") {
		...
		b.plugins.pluginloader = function(i, h) {
			...
			function n() {
				...
				if (!d && !k) {
					...
					for (var s in m) {
						var t = a.getPluginName(s), w = q[t], v = w.getJS(), u = w.getTarget(), p = w.getStatus();
			...
			this.setupPlugins = function(w, s, z) {
				...
				for (var v in s.plugins) {
					var x = a.getPluginName(v), y = t[x], A = y.getFlashPath(), B = y.getJS(), p = y.getURL();
					if (A) {
	...
	(function(b) {
		...
		var e = b.embed = function(t) {
			var p = new e.config(t.config), i, k, j = p.width, l = p.height, n = "Error loading player: ", h = b.plugins.loadPlugins(t.id, p.plugins);
			p.id = t.id;
				...
				if (h.getStatus() == a.loaderstatus.COMPLETE) {
					for (var w = 0; w < p.modes.length; w++) {
						if (p.modes[w].type && e[p.modes[w].type]) {
							var x = p.modes[w].config, u = a.extend({}, x ? e.config.addConfig(p, x) : p), v = new e[p.modes[w].type](i, p.modes[w], u, h, t);
							if (v.supportsConfig()) {
	...
	(function(e) {
		...
		g.download = function(n, y, l) {
			var q = j.extend({}, y), u, o = q.width ? q.width : 480, r = q.height ? q.height : 320, z, s, k = y.logo ? y.logo : {
				prefix : "http://p.jwpcdn.com/",
				file : "logo.png",
				margin : 10
			};
			function x() {
				var E, F, D, G, C = q.playlist, J, B, G, H = ["mp4", "aac", "mp3"];
				if (C && C.length) {
					J = C[0];
					B = J.sources;
					for ( G = 0; G < B.length; G++) {
						var A = B[G], I = A.type ? A.type : j.extensionmap.extType(j.extension(A.file));
						if (A.file) {
							for (G in H) {
								if (I == H[G]) {
									E = A.file;
	...
	(function(c) {
		...
		var e = c.embed.flash = function(l, m, q, k, o) {
			...
			this.embed = function() {
				...
				var E = "#000000", A, v = x.wmode ? x.wmode : (x.height && x.height <= 40 ? "transparent" : "opaque"), w = ["height", "width", "modes", "events", "primary", "base", "fallback"];
	...
	(function(d) {
		...
		f.destroyPlayer = function(j) {
			...
			if (i >= 0) {
				var n = k.id, h = g.getElementById(n + (k.renderingMode == "flash" ? "_wrapper" : ""));
				if (a.clearCss) {
					a.clearCss("#" + n)
				}
				if (h) {
